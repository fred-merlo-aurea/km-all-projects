...
this._owner.updateClientState();
k.get_column()._selectedCellsCount--;
}},_prepareCellsForSingleColumnSelection:function(s,m){var w=this;
var r=w._getGridTableFromElement(w._downTarget);
var t=r.parentNode;
var v=w._downTarget;
if(r!=null){var j;
var p=w._getHeaderSettings(r);
var o=p.getHeaderIndex(v);
if(m){j=a(t).find(String.format("{0}:nth-child({1})",w.options.filter,o+1));
w._setActiveOrUnselecting(j);
w._setActiveCellUponColumnSelection(j);
var u=w.get_currentTableView(v[0]);
var l=u.get_columns();
for(var q=0;
q<l.length;
q++){var k=l[q];
if(k.get_selected()){v=k.get_element();
j=a(t).find(String.format("{0}:nth-child({1})",w.options.filter,p.getHeaderIndex(v)+1));
w._setUnselecting(j);
}}}else{j=a(t).find(String.format("{0}:nth-child({1})",w.options.filter,o+1));
w._setActive(j);
var n=w.wrapper.find(w.options.filter+"."+w.options.styles.SELECTED).not(j);
w.selectable.unselect(n);
n.last().removeClass(f);
w._setActiveCellUponColumnSelection(j);
}}},_prepareCellsForMultiColumnSelection:function(t,k){var x=this;
var s=x._getGridTableFromElement(x._downTarget);
var u=s.parentNode;
if(s!=null){var j;
var v;
var o=x._getHeaderSettings(s);
var m;
if(t){if(x._lastActiveHeader==i||x._lastActiveHeader.length==0){v=x._downTarget;
m=o.getHeaderIndex(v);
j=a(u).find(String.format("{0}:nth-child({1})",x.options.filter,m+1));
x._setActiveOrUnselecting(j);
if(this._owner.ClientSettings.AllowKeyboardNavigation){x._setActiveCellUponColumnSelection(j);
}}else{if(x._lastActiveHeader[0]==x._downTarget[0]){return;
}else{v=x._downTarget;
var n;
x.selectable._shiftPressed=false;
if(o.hasMultiHeaders){var r=o.multiHeaders;
var w=a.inArray(v[0],r);
var q=a.inArray(x._lastActiveHeader[0],r);
if(w>q){n=r.slice(q+1,w+1);
}else{n=r.slice(w,q).reverse();
}}else{if(v.index()>x._lastActiveHeader.index()){n=x._lastActiveHeader.nextAll();
}else{n=x._lastActiveHeader.prevAll();
}}var p=0;
while(p<n.length){v=a(n[p]);
m=o.getHeaderIndex(v);
j=a(u).find(String.format("{0}:nth-child({1})",x.options.filter,m+1));
x._setActiveOrUnselecting(j);
if(v[0]===x._downTarget[0]){if(this._owner.ClientSettings.AllowKeyboardNavigation){x._setActiveCellUponColumnSelection(j);
}break;
}p++;
}}}}else{if(k){v=x._downTarget;
m=o.getHeaderIndex(v);
j=a(u).find(String.format("{0}:nth-child({1})",x.options.filter,m+1));
x._setActiveOrUnselecting(j);
if(this._owner.ClientSettings.AllowKeyboardNavigation){x._setActiveCellUponColumnSelection(j);
...
j=a(u).find(String.format("{0}:nth-child({1})",x.options.filter,m+1));
x._setActive(j);
var l=x.wrapper.find(x.options.filter+"."+x.options.styles.SELECTED).not(j);
x.selectable.unselect(l);
...
}},_selectColumnCells:function(o,l){var n=this;
var m=l.parentNode;
if(l==null){l=n._getGridTableFromElement(o);
}if(l){var k=n._getHeaderSettings(l).getHeaderIndex(o);
var j=a(m).find(String.format("{0}:nth-child({1})",n.options.filter,k+1));
n.selectable.value(j);
}},_deselectColumnCells:function(o,l){var n=this;
var m=l.parentNode;
if(l==null){l=n._getGridTableFromElement(o);
}if(l){var k=n._getHeaderSettings(l).getHeaderIndex(o);
var j=a(m).find(String.format("{0}:nth-child({1})",n.options.filter,k+1));
n.selectable.unselect(j);
}},_deselectTableViewCells:function(k){var m=this;
var l=k.parentNode;
if(k){if(k.get_element!=i){k=k.get_element();
}var j=a(l).find(String.format("{0}.{1}",m.options.filter,m.options.styles.SELECTED));
