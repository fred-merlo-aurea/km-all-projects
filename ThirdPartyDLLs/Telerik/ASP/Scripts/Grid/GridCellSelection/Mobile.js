...
this._owner.updateClientState();
e.get_column()._selectedCellsCount--;
}},_prepareCellsForSingleColumnSelection:function(n,g){var r=this;
var m=r._getGridTableFromElement(r._downTarget);
var o=m.parentNode;
var q=r._downTarget;
if(m!=null){var d;
var k=r._getHeaderSettings(m);
var j=k.getHeaderIndex(q);
if(g){d=a(o).find(String.format("{0}:nth-child({1})",r.options.filter,j+1));
r._setActiveOrUnselecting(d);
r._setActiveCellUponColumnSelection(d);
var p=r.get_currentTableView(q[0]);
var f=p.get_columns();
for(var l=0;
l<f.length;
l++){var e=f[l];
if(e.get_selected()){q=e.get_element();
d=a(o).find(String.format("{0}:nth-child({1})",r.options.filter,k.getHeaderIndex(q)+1));
r._setUnselecting(d);
}}}else{d=a(o).find(String.format("{0}:nth-child({1})",r.options.filter,j+1));
r._setActive(d);
var h=r.wrapper.find(r.options.filter+"."+r.options.styles.SELECTED).not(d);
r.selectable.unselect(h);
h.last().removeClass(b);
r._setActiveCellUponColumnSelection(d);
}}},_prepareCellsForMultiColumnSelection:function(o,e){var s=this;
var n=s._getGridTableFromElement(s._downTarget);
var p=n.parentNode;
if(n!=null){var d;
var q;
var j=s._getHeaderSettings(n);
var g;
if(o){if(s._lastActiveHeader==c||s._lastActiveHeader.length==0){q=s._downTarget;
g=j.getHeaderIndex(q);
d=a(p).find(String.format("{0}:nth-child({1})",s.options.filter,g+1));
s._setActiveOrUnselecting(d);
if(this._owner.ClientSettings.AllowKeyboardNavigation){s._setActiveCellUponColumnSelection(d);
}}else{if(s._lastActiveHeader[0]==s._downTarget[0]){return;
}else{q=s._downTarget;
var h;
s.selectable._shiftPressed=false;
if(j.hasMultiHeaders){var m=j.multiHeaders;
var r=a.inArray(q[0],m);
var l=a.inArray(s._lastActiveHeader[0],m);
if(r>l){h=m.slice(l+1,r+1);
}else{h=m.slice(r,l).reverse();
}}else{if(q.index()>s._lastActiveHeader.index()){h=s._lastActiveHeader.nextAll();
}else{h=s._lastActiveHeader.prevAll();
}}var k=0;
while(k<h.length){q=a(h[k]);
g=j.getHeaderIndex(q);
d=a(p).find(String.format("{0}:nth-child({1})",s.options.filter,g+1));
s._setActiveOrUnselecting(d);
if(q[0]===s._downTarget[0]){if(this._owner.ClientSettings.AllowKeyboardNavigation){s._setActiveCellUponColumnSelection(d);
}break;
}k++;
}}}}else{if(e){q=s._downTarget;
g=j.getHeaderIndex(q);
d=a(p).find(String.format("{0}:nth-child({1})",s.options.filter,g+1));
s._setActiveOrUnselecting(d);
if(this._owner.ClientSettings.AllowKeyboardNavigation){s._setActiveCellUponColumnSelection(d);
...
d=a(p).find(String.format("{0}:nth-child({1})",s.options.filter,g+1));
s._setActive(d);
var f=s.wrapper.find(s.options.filter+"."+s.options.styles.SELECTED).not(d);
s.selectable.unselect(f);
...
}},_selectColumnCells:function(i,f){var h=this;
var g=f.parentNode;
if(f==null){f=h._getGridTableFromElement(i);
}if(f){var e=h._getHeaderSettings(f).getHeaderIndex(i);
var d=a(g).find(String.format("{0}:nth-child({1})",h.options.filter,e+1));
h.selectable.value(d);
}},_deselectColumnCells:function(i,f){var h=this;
var g=f.parentNode;
if(f==null){f=h._getGridTableFromElement(i);
}if(f){var e=h._getHeaderSettings(f).getHeaderIndex(i);
var d=a(g).find(String.format("{0}:nth-child({1})",h.options.filter,e+1));
h.selectable.unselect(d);
}},_deselectTableViewCells:function(e){var g=this;
var f=e.parentNode;
if(e){if(e.get_element!=c){e=e.get_element();
}var d=a(f).find(String.format("{0}.{1}",g.options.filter,g.options.styles.SELECTED));
