...
(function(b,a){a("dataviz/map/location",["kendo.drawing"],b);
}(function(){(function(a,v){var n=Math,b=n.abs,c=n.atan,d=n.atan2,f=n.cos,o=n.max,p=n.min,s=n.sin,u=n.tan,l=window.kendo,e=l.Class,g=l.dataviz,h=l.deepExtend,w=l.util,i=w.defined,j=w.deg,q=w.rad,r=w.round,t=w.sqr,x=w.valueOrDefault;
...
}(function(){(function(a,u){var k=Math,m=a.proxy,h=window.kendo,b=h.Class,q=h.template,c=h.dataviz,d=h.deepExtend,e=h.geometry,l=e.Point,i=c.map.layers.Layer,v=h.util,o=v.round,n=v.renderSize,j=v.limitValue;
var r=i.extend({init:function(g,w){i.fn.init.call(this,g,w);
if(typeof this.options.subdomains==="string"){this.options.subdomains=this.options.subdomains.split("");
}var x=this._viewType();
this._view=new x(this.element,this.options);
},destroy:function(){i.fn.destroy.call(this);
this._view.destroy();
this._view=null;
},_beforeReset:function(){var g=this.map;
var w=g.locationToLayer(g.extent().nw).round();
this._view.viewOrigin(w);
},_reset:function(){i.fn._reset.call(this);
this._updateView();
this._view.reset();
},_viewType:function(){return t;
},_activate:function(){i.fn._activate.call(this);
if(!h.support.mobileOS){if(!this._pan){this._pan=h.throttle(m(this._render,this),100);
}this.map.bind("pan",this._pan);
}},_deactivate:function(){i.fn._deactivate.call(this);
if(this._pan){this.map.unbind("pan",this._pan);
}},_updateView:function(){var y=this._view,x=this.map,g=x.extent(),w={nw:x.locationToLayer(g.nw).round(),se:x.locationToLayer(g.se).round()};
y.center(x.locationToLayer(x.center()));
y.extent(w);
y.zoom(x.zoom());
...
var t=b.extend({init:function(g,w){this.element=g;
this._initOptions(w);
this.pool=new s();
...
},pointToTileIndex:function(g){return new l(k.floor(g.x/this.options.tileSize),k.floor(g.y/this.options.tileSize));
},tileCount:function(){var y=this.size(),g=this.pointToTileIndex(this._extent.nw),w=this._extent.nw,x=this.indexToPoint(g).translate(-w.x,-w.y);
return{x:k.ceil((k.abs(x.x)+y.width)/this.options.tileSize),y:k.ceil((k.abs(x.y)+y.height)/this.options.tileSize)};
},size:function(){var w=this._extent.nw,x=this._extent.se,g=x.clone().translate(-w.x,-w.y);
return{width:g.x,height:g.y};
},indexToPoint:function(g){var w=g.x,z=g.y;
return new l(w*this.options.tileSize,z*this.options.tileSize);
...
this.render();
},render:function(){var w=this.tileCount(),g=this.pointToTileIndex(this._extent.nw),z,A,B;
for(A=0;
A<w.x;
A++){for(B=0;
B<w.y;
B++){z=this.createTile({x:g.x+A,y:g.y+B});
if(!z.visible){z.show();
}}}},createTile:function(g){var w=this.tileOptions(g);
var x=this.pool.get(this._center,w);
if(x.element.parent().length===0){this.element.append(x.element);
}return x;
},tileOptions:function(g){var w=this.wrapIndex(g),z=this.indexToPoint(g),y=this._viewOrigin,x=z.clone().translate(-y.x,-y.y);
return{index:w,currentIndex:g,point:z,offset:p(x),zoom:this._zoom,size:this.options.tileSize,subdomain:this.subdomainText(),urlTemplate:this.options.urlTemplate,errorUrlTemplate:this.options.errorUrlTemplate};
},wrapIndex:function(w){var g=k.pow(2,this._zoom);
return{x:this.wrapValue(w.x,g),y:j(w.y,0,g-1)};
},wrapValue:function(x,g){var w=k.abs(x)%g;
if(x>=0){x=w;
}else{x=g-(w===0?g:w);
}return x;
}});
var f=b.extend({init:function(g,w){this.id=g;
this.visible=true;
this._initOptions(w);
this.createElement();
this.show();
},options:{urlTemplate:"",errorUrlTemplate:""},createElement:function(){this.element=a("<img style='position: absolute; display: block;' />").css({width:this.options.size,height:this.options.size}).error(m(function(g){if(this.errorUrl()){g.target.setAttribute("src",this.errorUrl());
}else{g.target.removeAttribute("src");
}},this));
},show:function(){var g=this.element[0];
g.style.top=n(this.options.offset.y);
g.style.left=n(this.options.offset.x);
var w=this.url();
if(w){g.setAttribute("src",w);
}g.style.visibility="visible";
this.visible=true;
},hide:function(){this.element[0].style.visibility="hidden";
this.visible=false;
},url:function(){var g=q(this.options.urlTemplate);
return g(this.urlOptions());
},errorUrl:function(){var g=q(this.options.errorUrlTemplate);
return g(this.urlOptions());
...
var s=b.extend({init:function(){this._items=[];
},options:{maxSize:100},get:function(g,w){if(this._items.length>=this.options.maxSize){this._remove(g);
}return this._create(w);
},empty:function(){var w=this._items;
for(var g=0;
g<w.length;
g++){w[g].destroy();
}this._items=[];
},reset:function(){var w=this._items;
for(var g=0;
g<w.length;
g++){w[g].hide();
}},_create:function(y){var x=this._items;
var z;
var w=v.hashKey(y.point.toString()+y.offset.toString()+y.zoom+y.urlTemplate);
for(var g=0;
g<x.length;
g++){if(x[g].id===w){z=x[g];
break;
...
},_remove:function(g){var z=this._items;
var A=-1;
var y=-1;
for(var x=0;
x<z.length;
x++){var w=z[x].options.point.distanceTo(g);
if(w>A&&!z[x].visible){y=x;
A=w;
}}if(y!==-1){z[y].destroy();
z.splice(y,1);
}}});
function p(g){return new l(o(g.x),o(g.y));
}d(c,{map:{layers:{tile:r,TileLayer:r,ImageTile:f,TilePool:s,TileView:t}}});
}(window.kendo.jQuery));
},typeof define=="function"&&define.amd?define:function(a,b,c){(c||b)();
}));
...
this.trigger("reset");
},_resetScroller:function(){var F=this.scroller;
var I=F.dimensions.x;
var K=F.dimensions.y;
var E=this._layerSize();
var g=this.extent().nw;
var G=this.locationToLayer(g).round();
F.movable.round=true;
F.reset();
F.userEvents.cancel();
var M=this.zoom();
F.dimensions.forcedMinScale=v(2,this.options.minZoom-M);
F.dimensions.maxScale=v(2,this.options.maxZoom-M);
var J={min:-G.x,max:E-G.x};
var L={min:-G.y,max:E-G.y};
if(this.options.wraparound){J.max=20*E;
J.min=-J.max;
}if(this.options.pannable===false){var H=this.viewSize();
J.min=L.min=0;
J.max=H.width;
L.max=H.height;
}I.makeVirtual();
K.makeVirtual();
I.virtualSize(J.min,J.max);
K.virtualSize(L.min,L.max);
this._virtualSize={x:J,y:L};
