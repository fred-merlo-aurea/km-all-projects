(function(b,a){a("kendo.timepicker",["kendo.popup"],b);
...
(function(b,V){var z=window.kendo,A=z.keys,I=z.parseDate,c=z._activeElement,r=z._extractFormat,Q=z.support,i=Q.browser,U=z.ui,W=U.Widget,H="open",l="close",j="change",G=".kendoTimePicker",k="click"+G,n="k-state-default",o="disabled",L="readonly",B="li",O="<span/>",s="k-state-focused",u="k-state-hover",v="mouseenter"+G+" mouseleave"+G,C="mousedown"+G,E=60000,D=86400000,M="k-state-selected",P="k-state-disabled",h="aria-selected",f="aria-expanded",g="aria-hidden",e="aria-disabled",d="aria-activedescendant",w="id",x=b.isArray,q=b.extend,K=b.proxy,m=Date,T=new m();
T=new m(T.getFullYear(),T.getMonth(),T.getDate(),0,0,0);
var S=function(Y){var Z=this,X=Y.id;
Z.options=Y;
Z._dates=[];
Z.ul=b('<ul tabindex="-1" role="listbox" aria-hidden="true" unselectable="on" class="k-list k-reset"/>').css({overflow:Q.kineticScrollNeeded?"":"auto"}).on(k,B,K(Z._click,Z)).on("mouseenter"+G,B,function(){b(this).addClass(u);
}).on("mouseleave"+G,B,function(){b(this).removeClass(u);
});
Z.list=b("<div class='k-list-container k-list-scroller' unselectable='on'/>").append(Z.ul).on(C,J);
...
}if(Y){Y=b(Y).addClass(M).attr(w,Z._optionID).attr(h,true);
Z.scroll(Y[0]);
}Z._current=Y;
if(X){X(Y);
...
Y.current(null);
Y.select(Y._value);
},scroll:function(ab){if(!ab){return;
}var Y=this.list[0],ad=ab.offsetTop,ac=ab.offsetHeight,aa=Y.scrollTop,Z=Y.clientHeight,X=ad+ac;
if(aa>ad){aa=ad;
}else{if(X>aa+Z){aa=X-Z;
}}Y.scrollTop=aa;
},select:function(Y){var ab=this,Z=ab.options,X=ab._current,aa;
if(Y instanceof Date){Y=z.toString(Y,Z.format,Z.culture);
}if(typeof Y==="string"){if(!X||X.text()!==Y){Y=b.grep(ab.ul[0].childNodes,function(ac){return(ac.textContent||ac.innerText)==Y;
});
Y=Y[0]?Y:null;
}else{Y=X;
...
}},_click:function(Z){var ab=this,aa=b(Z.currentTarget),X=aa.text(),Y=ab.options.dates;
...
ad.attr(g,true);
}},active:function(ae){Z.removeAttr(d);
if(ae){Z.attr(d,ac._optionID);
}}}));
ad=ac.ul;
ab._icon();
ab._reset();
...
X=Z.is("[disabled]")||b(ab.element).parents("fieldset").is(":disabled");
...
Z.removeAttr(o).removeAttr(L).attr(e,false).on("keydown"+G,K(ac._keydown,ac)).on("focusout"+G,K(ac._blur,ac)).on("focus"+G,function(){ac._inputWrapper.addClass(s);
});
X.on(k,K(ac._click,ac)).on(C,J);
}else{ad.addClass(Y?P:n).removeClass(Y?n:P);
Z.attr(o,Y).attr(L,ab).attr(e,Y);
...
if(!X[0]){X=b('<span unselectable="on" class="k-select" aria-label="select"><span class="k-icon k-i-clock"></span></span>').insertAfter(Y);
...
Z.timeView.bind();
},_toggleHover:function(X){b(X.currentTarget).toggleClass(u,X.type==="mouseenter");
},_update:function(ab){var Z=this,Y=Z.options,aa=Z.timeView,X=aa._parse(ab);
if(!y(X,Y.min,Y.max)){X=null;
}Z._value=X;
Z.element.val(z.toString(X||ab,Y.format,Y.culture));
aa.value(X);
return X;
...
Y._inputWrapper=b(Z[0].firstChild);
},_reset:function(){var aa=this,X=aa.element,Z=X.attr("form"),Y=Z?b("#"+Z):X.closest("form");
