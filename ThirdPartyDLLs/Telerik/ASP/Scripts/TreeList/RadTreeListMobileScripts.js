...
Telerik.Web.UI.RadTreeList.prototype={initialize:function(){var k;
var h;
...
}var c=$telerik.$(this.get_element()).find("div.rtlCommand > button.rtlMenu");
var b=$telerik.$(this.get_element()).find("div.rtlCommand > button.rtlExport");
if(c){c.offEvent(a.EventNamesMap.up).onEvent(a.EventNamesMap.up,$telerik.$.proxy(this._menuButtonClick,this)).on("click",$telerik.preventDefault);
}if(b){b.offEvent(a.EventNamesMap.up).onEvent(a.EventNamesMap.up,$telerik.$.proxy(this._exportButtonClick,this)).on("click",$telerik.preventDefault);
...
if($telerik.isTouchDevice&&this._clientSettings&&this._clientSettings._scrolling&&this._clientSettings._scrolling._allowScroll){h=$get(this.get_id()+"_rtlData");
this._dataDivMouseDownDelegate=$telerik.addMobileHandler(this,h,"mousedown",this._dataDivMouseDown);
this._dataDivMouseUpDelegate=$telerik.addMobileHandler(this,h,"mouseup",this._dataDivMouseUp);
}if($telerik.isMobileIE10&&this._clientSettings&&(this._clientSettings._allowItemsDragDrop||this.get_events().getHandler("itemDblClick"))){if(this._clientSettings._scrolling&&this._clientSettings._scrolling._allowScroll){h=$get(this.get_id()+"_rtlData");
...
}var l=this.get_element().getElementsByTagName("table")[0];
var j=this._getTableHeaderRow(l);
...
var e=j.cells[k];
if(!e){continue;
}var f=$create(Telerik.Web.UI.TreeListColumn,{_data:g},null,{owner:this.get_clientID()},j.cells[k]);
...
if(d){d.show();
}}},_exportButtonClick:function(b){var c=b.target||b.srcElement;
if(c.className&&c.className.indexOf("rtlExport")>-1){var d=this._getViewByType(a.TreeList.MobileViewType.Export);
if(d){d.show();
...
var b=document.body;
d.style.width=Math.max(Math.max(c.scrollWidth,b.scrollWidth),Math.max(c.offsetWidth,b.offsetWidth))+"px";
