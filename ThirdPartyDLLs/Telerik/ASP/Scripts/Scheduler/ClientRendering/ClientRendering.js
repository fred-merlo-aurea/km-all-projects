...
if(!t.get_readOnlyResolved()){if(s){r.append("</div><a href='#' class='rsAptDelete' style='visibility: hidden;' title='delete'><span class='p-icon p-i-x'></span></a>").append(u?"<span class='rsAptResize rsAptResizeStart'></span><span class='rsAptResize rsAptResizeEnd'></span>":"");
...
var p=this._isClassic;
if(q!=d.RecurrenceState.NotRecurring){if(q==d.RecurrenceState.Exception){return p?'<div class="rsAptRecurrenceException"></div>':'<span class="rsAptRecurrenceException"><span class="p-icon p-i-recurrence-exception"></span></span>';
}return p?'<div class="rsAptRecurrence"></div>':'<span class="rsAptRecurrence"><span class="p-icon p-i-recurrence"></span></span>';
}return"";
},_getReminderIconHtml:function(o){if(o.get_reminders&&o.get_reminders().get_count()>0){return this._isClassic?'<div class="rsAptReminder"></div>':'<span class="rsAptReminder"><span class="p-icon p-i-reminder"></span></span>';
}return"";
...
if(this._isLightweight){p=String.format('<a style="z-index: 80;" href="#" class="rsArrow{0}" title="{1}"><span class="p-icon p-i-arrow-60-{0}"></span></a>',r,r.toLowerCase());
a(o).append(p);
return;
