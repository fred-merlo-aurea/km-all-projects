...
d.add_show(function(n){e.updateClientState();
var i=e.get_configurationPanel();
if(!i||i.get_element().parentNode.className.indexOf("rpgFieldsWindow")==-1){if(e._dragging){e._dragging._attachFieldsWindowEvent();
}}else{if(i._enableFieldsContextMenu){g._onFieldsWindowContextMenuDelegates=$telerik.addMobileHandler(g,n.get_contentElement(),"contextmenu",g._onContextMenuHandler);
}g._onFieldsWindowClickDelegate=$telerik.addMobileHandler(g,n.get_contentElement(),"click",g._onClickHandler);
}g._isFirstWindowShow=false;
f.set_text(e._hideFieldsWindowText);
if(e._materialRippleManager){var m={element:d.get_contentElement(),rippleConfigurations:[{containerSelector:".rpgUpdate button.rpgUpdateButton"},{containerSelector:".rpgConfigurationPanel a.rpgChangeLayoutIcon",rippleType:Telerik.Web.UI.MaterialRippleType.Icon,maxRippleSize:50}]};
var h=null;
var j=e.get_id()+"_OT";
var l=$telerik.$("#"+j).find("[id*='SortLinkButton']").length>0;
var k=$telerik.$("#"+j).find(".rpgFilterIcon").length>0;
if(e.get_configurationPanel()&&(e.get_configurationPanel()._enableDragDrop||l||k)){if(e.get_configurationPanel()._enableDragDrop){h=".rpgConfigurationPanel .rpgFieldItem";
}else{if(l||k){h=".rpgConfigurationPanel .rpgRowFieldsContainer .rpgFieldItem, .rpgConfigurationPanel .rpgColumnFieldsContainer .rpgFieldItem";
if(k){h+=", .rpgConfigurationPanel .rpgAllFieldsContainer .rpgFieldItem";
}}}m.rippleConfigurations.push({containerSelector:h});
}e._materialRippleManager.initializeRippleZone(m);
}});
