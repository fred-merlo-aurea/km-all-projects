...
namespace UAS.UnitTests.Helpers
{
    ...
    public static class ReflectionHelper
    {
            ...
            object instance = null)
        {
            ...
            MethodInfo methodInfo = type.GetMethod(methodName, BindingFlags(), Type.DefaultBinder, parameterTypes, null);
            if (methodInfo != null)
            {
                if (methodInfo.ReturnType != typeof(void))
                {
        ...
        public static TReturnType GetPropertyValueAs<TReturnType>(this object instance, string propertyName) where TReturnType : class
        {
            return GetPropertyValue(instance, propertyName) as TReturnType;
        }
        ...
        public static TReturnType GetFieldValueAs<TReturnType>(this object instance, string fieldName) where TReturnType : class
        {
            return GetFieldValue(instance, fieldName) as TReturnType;
        }
        ...
        /// <summary>
        ///  Generate Sql Data Reader from List
        /// </summary>
        /// <typeparam name="T"></typeparam>
        /// <param name="list"></param>
        /// <returns></returns>
        public static SqlDataReader GetSqlDataReader<T>(this IList<T> list)
        {
            if (list == null)
            {
            ...
            // Action is called when index is incremented
            ShimDynamicBuilder<T>.AllInstances.BuildIDataRecord = (builder, record) => list[index];
            return new ShimSqlDataReader
            {
                ...
                GetOrdinalString = name => typeof(T).GetPropertyIndex(name),
                ItemGetInt32 = x => list[index].GetPropertyValue(x),
                IsDBNullInt32 = x =>
                {
                    var propertyValue = list[index].GetPropertyValue(x);
