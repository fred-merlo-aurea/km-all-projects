namespace Core_AMS.Utilities
{
    ...
    public class AddressParser
    {
        ...
        private const string CharsPattern = @"(\w)";
        private const string ZipPattern = @"\d{5}(?:-?\d{4})?";
        private const string StatePatternTemplate = @"\b(?:{0})\b";
        private const string RangedSecondaryUnitPatternTemplate = @"(?<SECONDARYUNIT>{0})(?![a-z])";
        private const string RangelessSecondaryUnitPatternTemplate = @"(?<SECONDARYUNIT>{0})\b";
        ...
        private const string StreetPatternTemplate = @"
                        (?:
        ...
        private const string SecondaryUnitPatternTemplate = @"
                    (
                        (:?
                            (?: (?:{0} \W*)
                                | (?<SECONDARYUNIT>\#)\W*
        ...
        private const string PlacePatternTemplate = @"
                    (?:{0}\W*)?
                    (?:(?<ZIP>{1}))?";

        private const string CityAndStatePatternTemplate = @"
                    (?:
                        (?<CITY>[^\d,]+?)\W+
                        (?<STATE>{0})
                    )";

        private const string AddressPatternTemplate = @"
                    ^
        ...
        private static void InitializeRegex()
        {
            var addressPattern = string.Format(
                ...
                AddressPatternTemplate,
                NumberPattern,
                GetStreetPattern(),
                GetSecondaryUnitPattern(),
                GetPlacePattern(),
                ZipPattern);
        ...
        private static string GetPlacePattern()
        {
            var statePatternValues = string.Join(
                Separator,  
                string.Join(Separator, states.Keys.Select(Regex.Escape)), 
                string.Join(Separator, states.Values));
            
            var cityAndStatePattern = string.Format(
                CultureInfo.InvariantCulture, 
                CityAndStatePatternTemplate, 
                string.Format(CultureInfo.InvariantCulture, StatePatternTemplate, statePatternValues));

            return string.Format(CultureInfo.InvariantCulture, PlacePatternTemplate, cityAndStatePattern, ZipPattern);
        }

        private static string GetSecondaryUnitPattern()
        {
            var rangedSecondaryUnitPattern = string.Format(
                    CultureInfo.InvariantCulture, 
                    RangedSecondaryUnitPatternTemplate, 
                    string.Join(Separator, rangedSecondaryUnits.Keys));

            var rangelessSecondaryUnitPattern = string.Format(
                    CultureInfo.InvariantCulture, 
                    RangelessSecondaryUnitPatternTemplate,
                    string.Join(Separator, string.Join(Separator, rangelessSecondaryUnits.Keys)));

            return string.Format(
                CultureInfo.InvariantCulture,
                SecondaryUnitPatternTemplate,
                rangedSecondaryUnitPattern,
                rangelessSecondaryUnitPattern);
        }

        private static string GetStreetPattern()
        {
            var directionalPattern = string.Join(
                Separator,
                string.Join(Separator, directionals.Keys),
                string.Join(Separator, directionals.Values),
                string.Join(Separator, directionals.Values.Select(x => Regex.Replace(x, CharsPattern, @"$1\."))));

            var suffixPattern = new Regex(
                string.Join(Separator,
                    string.Join(Separator, suffixes.Keys),
                    string.Join(Separator, suffixes.Values.Distinct())),
                RegexOptions.Compiled);

            return string.Format(CultureInfo.InvariantCulture, StreetPatternTemplate, directionalPattern, suffixPattern);
        }
