...
Begin   
	...
	select @PublicationID = PublicationID, @ResponseType=Row from PublicationReports where reportID = @reportID  
	...
	begin  
		insert into #responseID select items from dbo.fn_Split(@RowID,',')   
	end
	
	if @Download  = '0'
	Begin

		select	@distinctcount = sum(copies) FROM  #SubscriptionID sf   
		...
		from 
			#SubscriptionID s left outer join 
			(
				select subid as SubscriptionID from #srm 
		...
		from #cte cte  join
			(select DisplayName, count(distinct subscriptionID) as groupUniques from #cte ct2 group by DisplayName) as inn2 on cte.DisplayName  = inn2.displayname
