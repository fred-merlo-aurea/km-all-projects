...
Begin
	...
	Begin
		...
		from 
				...
				'+@db+'..SubscriptionResponseMap srm on t.SubscriptionID = srm.SubscriptionID join 
				vw_Response r on r.ResponseID = srm.ResponseID and r.PublicationID = '+CAST(@PublicationID as varchar(25))+'
		...
		select  
	 			@DemoColumns = @DemoColumns + coalesce('[' + RTRIM(displayname)  + '] as ''' + RTRIM(displayname)  + ''',','') ,
	...
	Begin
		...
		set @demofieldsstring= @demofieldsstring + ' left outer join SubscriptionPaid spd on spd.SubscriptionID = s.subscriptionID'
	End
	...
	exec (' select distinct ' + @profilefields +  @DemoColumns + 
			...
			' join Subscriber sb on sb.SubscriberID = s.SubscriberID ' + 
			@demofieldsstring
			)
