CREATE PROCEDURE [dbo].[o_ExportData_Select]
AS
BEGIN

	SET NOCOUNT ON

	SELECT DISTINCT s.SubscriberID, ss.PublicationID,
		...
		s.FirstName as 'Fname', s.LastName as 'Lname', s.Title, s.Company, s.Address1 as 'Address', s.Address2 as 'Mailstop', s.City, s.RegionCode as 'State', s.ZipCode as 'ZipCode', s.Plus4,
		s.County, c.CountryName as 'Country', c.CountryID as 'CountryID', s.Phone, s.Fax, s.Mobile, s.Email, s.Website, ss.AccountNumber as 'AcctNum',
		...
		,sp.PaidDate as 'PAYDATE', sp.StartIssueDate as 'STARTISS', sp.ExpireIssueDate as 'EXPIRE' ,ci.NewExpire as 'NWEXPIRE'
   FROM Subscriber s WITH(NoLock)
	   ...
	   FULL OUTER JOIN UAD_Lookup..Action a WITH(NoLock)
		   ON ss.ActionID_Current = a.ActionID
	   FULL OUTER JOIN UAD_Lookup..CategoryCode cc WITH(NoLock)
		   ON a.CategoryCodeID = cc.CategoryCodeID
	   FULL OUTER JOIN UAD_Lookup..TransactionCode tc WITH(NoLock)
		   ON a.TransactionCodeID = tc.TransactionCodeID
	   FULL OUTER JOIN UAD_Lookup..Country c WITH(NoLock)
