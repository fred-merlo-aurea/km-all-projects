...
BEGIN
	...
	SET NOCOUNT ON
	----Start Test Data--------------------------------------
	--DECLARE @ClientName varchar(100) = 'Name', @PubSubscriptionID int = 1
	----End Test Data----------------------------------------
		...
		Select UserLogID, UserLogTypeID, [Object], FromObjectValues, ToObjectValues, DateCreated
			...
			(
				...
				from Batch b With(NoLock)
					...
					where h.PubSubscriptionID = @PubSubscriptionID and htul.UserLogID is not null		
			)
	END
	else
	BEGIN
	...
	SELECT b.BatchID,b.UserID,b.BatchCount,b.BatchNumber,b.BatchNumber,
		   b.IsActive,b.DateCreated as 'BatchDateCreated',
		   b.DateFinalized as 'BatchDateFinalized',
		   h.PublicationID, p.PubName, p.PubCode,
	...
	WHERE s.PubSubscriptionID = @PubSubscriptionID and htul.UserLogID != h.HistorySubscriptionID
