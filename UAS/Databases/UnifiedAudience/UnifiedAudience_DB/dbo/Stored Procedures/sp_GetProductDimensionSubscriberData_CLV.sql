CREATE PROCEDURE [dbo].[sp_GetProductDimensionSubscriberData_CLV]
@Queries XML,
...
BEGIN
	...
	CREATE TABLE #tmpSubscriptionID (ID int identity(1,1), subsID int, CGRP_No varchar(100));
    ...
    CREATE TABLE #tmpPubSubscriptionDetails (CGRP_No uniqueidentifier, DisplayName VARCHAR(250), ResponseValue VARCHAR(255)); 
	CREATE TABLE #tmpPubSubscriptionDetailsDesc (CGRP_No uniqueidentifier, DisplayName VARCHAR(250), ResponseValue VARCHAR(255), ResponseDesc VARCHAR(255)); 
    ...
    CREATE INDEX IDX_tmpPubSubscriptionDetails_CGRP_No ON #tmpPubSubscriptionDetails(CGRP_No)
    CREATE INDEX IDX_tmpPubSubscriptionDetailsDesc_subscriptionID ON #tmpPubSubscriptionDetailsDesc(CGRP_No)
	...
	Begin
		insert into #tmpSubscriptionID
		select SubscriptionID, Cgrp_no
		from
		(
			select  
				s.SubscriptionID, s.Cgrp_no,  row_number() over(partition by Cgrp_no order by Qdate desc) as rn
			from 
				#TempSubs t join 
				Subscriptions s with (nolock) on s.SubscriptionID = t.subsID
		) O1
...
SELECT	@scolumns = COALESCE(@scolumns + ',' + 
    ...
    Else
		Begin
			...
			select distinct t.CGRP_No, rg.DisplayName,  c.Responsevalue 
			...
			select distinct t.CGRP_No, rg.DisplayName + '_Description',  c.Responsevalue,  
