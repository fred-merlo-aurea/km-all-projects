			...
			if (@IsDownload = 0)
				begin
					...
					from Subscription s  WITH(NOLOCK)
						join Subscriber sb WITH(NOLOCK) on s.SubscriberID = sb.subscriberID 
						join Action a WITH(NOLOCK) on a.ActionID = s.ActionID_Current 
						join CategoryCode cc WITH(NOLOCK) on cc.CategoryCodeID  = a.CategoryCodeID 
						join TransactionCode tc WITH(NOLOCK) on tc.TransactionCodeID = a.TransactionCodeID  
						left outer join QualificationSource qs WITH(NOLOCK)  on qs.QSourceID = s.QSourceID
		...
		Begin
			...
			if (@IsDownload = 0)
				...
				set @executeString =' Select	count(s.SubscriptionID) as count ' +
									' from	Subscription s with(nolock) join ' +
											' Subscriber sb with(nolock) on s.SubscriberID = sb.subscriberID join ' +
											' Action a with(nolock) on a.ActionID = s.ActionID_Current join ' +
											' CategoryCode cc with(nolock) on cc.CategoryCodeID  = a.CategoryCodeID join ' +
											' TransactionCode tcw ith(nolock)  on tc.TransactionCodeID = a.TransactionCodeID  left outer join  ' +
											' QualificationSource qs with(nolock) on qs.QSourceID = s.QSourceID left outer join ' +
											' dbo.Deliverability db with(nolock) on db.DeliverabilityID = s.DeliverabilityID left outer join ' +
											'(
				...
				begin
					set @executeString =' Select distinct s.SubscriptionID, s.copies ' +
										' from	Subscription s with(nolock) join ' +
												...
												' Action a with(nolock) on a.ActionID = s.ActionID_Current join ' +
												' CategoryCode cc with(nolock) on cc.CategoryCodeID  = a.CategoryCodeID join ' +
												' TransactionCode tc with(nolock) on tc.TransactionCodeID = a.TransactionCodeID  left outer join  ' +
												' SubscriptionPaid sp with(nolock) on sp.SubscriptionID = s.SubscriptionID left outer join' +
												' dbo.Deliverability db with(nolock) on db.DeliverabilityID = s.DeliverabilityID left outer join ' +
												'(
													SELECT SubscriberID, ISNULL(ML, 0) as Demo31, ISNULL(FX, 0) as Demo32, ISNULL(PH, 0) as Demo33, ISNULL(TS, 0) as Demo34, ISNULL(ADV, 0) as Demo35, ISNULL(EM, 0) as Demo36
													FROM
