CREATE PROCEDURE [dbo].[ccp_Vcast_Process_File_MX_Elan]
@Xml xml
AS
BEGIN
	SET NOCOUNT ON  	
	...
	Insert into tempVcastMXElan(SubNum,SO2,SO3)
	SELECT SUBSCRBNUM,SUB02ANS,SUB03ANS FROM @import
	
		
	INSERT INTO tempVcastMXElanFinal (SubNum,SO2,SO3)	
	SELECT Distinct SubNum,
		[UAS].[DBO].[Remove_Dup_Entry](STUFF((SELECT ',' + SUB.SO2 AS [text()] FROM tempVcastMXElan SUB
		WHERE SUB.SubNum = MAIN.SubNum For Xml Path(''), type
		).value('.', 'nvarchar(max)')
		, 1, 1, ''), ',') As SO2,
		[UAS].[DBO].[Remove_Dup_Entry](STUFF((SELECT ',' + SUB.SO3 AS [text()] FROM tempVcastMXElan SUB
		WHERE SUB.SubNum = MAIN.SubNum For Xml Path(''), type
		).value('.', 'nvarchar(max)')
		, 1, 1, ''), ',') As SO3
	FROM tempVcastMXElan MAIN With(NoLock)
	GROUP BY SubNum
