...
BEGIN
	...
	where SORecordIdentifier in (select so.SORecordIdentifier from SubscriberOriginal so where ProcessCode = @processCode)

	delete SubscriberDemographicArchive 
	where SARecordIdentifier in (select sa.SARecordIdentifier from  SubscriberArchive sa where ProcessCode = @processCode)

	delete SubscriberDemographicFinal 
	where SFRecordIdentifier in (select sf.SFRecordIdentifier from  SubscriberFinal sf where ProcessCode = @processCode)

	delete SubscriberDemographicTransformed 
	where STRecordIdentifier in (select st.STRecordIdentifier from SubscriberTransformed st where ProcessCode = @processCode)
