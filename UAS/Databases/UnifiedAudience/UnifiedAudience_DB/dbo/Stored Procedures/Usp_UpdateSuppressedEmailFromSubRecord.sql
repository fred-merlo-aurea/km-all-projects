...
BEGIN
	...
	WHERE ps.EmailstatusID = @MasterSuppressedId;


	UPDATE s
	SET EmailExists = 0, Email = ''
	FROM Subscriptions s
		JOIN Pubsubscriptions ps ON s.Subscriptionid = ps.subscriptionid AND s.Email = ps.Email
	WHERE ps.EmailstatusID = @MasterSuppressedId;
	 ...
	 FROM PubSubscriptions ps
		JOIN (	SELECT SubscriptionId,
				...
				WHERE EmailstatusId =@ActiveId
					AND ISNULL(Email,'') <> ''
				GROUP BY SubscriptionID) ae ON ps.subscriptionid = ae.subscriptionId AND ps.StatusUpdatedDate = ae.StatusUpdatedDate
	 WHERE Emailstatusid = @ActiveId)

	UPDATE S
	SET Email = cte_ActiveEmail.Email, EmailExists = 1
	FROM subscriptions s 
	...
	SELECT DISTINCT ps.SubscriptionId,
		...
		ps.StatusUpdatedDate
	 FROM PubSubscriptions ps
		JOIN (SELECT SubscriptionId, MAX(StatusUpdatedDate) AS StatusUpdatedDate 
			FROM PubSubscriptions 
			WHERE --EmailstatusId = @MasterSuppressedId
					ISNULL(Email,'') <> ''
			GROUP BY SubscriptionID) ae ON ps.subscriptionid = ae.subscriptionId AND ps.StatusUpdatedDate = ae.StatusUpdatedDate
	...
	UPDATE S
	SET Email = cte_RemainingEmail.Email, EmailExists = 1
	FROM subscriptions s 
