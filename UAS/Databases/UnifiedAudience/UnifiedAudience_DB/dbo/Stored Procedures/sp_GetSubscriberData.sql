CREATE PROCEDURE [dbo].[sp_GetSubscriberData]
@Queries XML,
@StandardColumns XML,
...
BEGIN
	...
	CREATE TABLE #TempSubscription (ID int identity(1,1), subsID int);
	...
	if (@SubscriptionID.exist('/Subscriptions') != 0)
	Begin
		insert into #TempSubscription
		SELECT SubscriptionID.ID.value('./@SubscriptionID','INT')
		FROM @SubscriptionID.nodes('/Subscriptions') as SubscriptionID(ID) ;	
	End
	Else
	Begin
		declare @hDoc AS INT
		...
		select @TotalRecords = COUNT(ID) from #TempSubscription with (NOLOCK)
	
		if @DownloadCount > 0 and @DownloadCount <> @TotalRecords
		Begin
			set @nth = @TotalRecords/@DownloadCount
		
			--select @nth as nth, @TotalRecords as tr, @DownloadCount dc
			--select ID % @nth, * from #TempSubscription
		
			if @nth <> 0 and @nth <> 1
			Begin
				delete from #TempSubscription
				where @DownloadCount <> -1 and ID % @nth <> 0
			end
		end
		--select * from #TempSubscription
	End	
						...
						(
							...
							(
								select  
										...
										soa.activitydate, 
										psa.pubID,
										ROW_NUMBER() over (partition by soa.subscriptionID order by activitydate desc) as RN
								from 
										#TempSubscription ts join
										SubscriberOpenActivity soa with (NOLOCK) on soa.subscriptionID = ts.subsID
										left outer join PubSubscriptions psa with (NOLOCK) on soa.pubsubscriptionId = psa.pubsubscriptionId '
	
				if len(ltrim(rtrim(@PubIDs))) > 0
					set @LastOpenQuery = @LastOpenQuery + ' where psa.pubID in (' + @PubIDs + ') ' 
	
				set @LastOpenQuery = @LastOpenQuery + ' ) t left outer join Pubs p  with (NOLOCK) on t.PubID = p.PubID
							where t.RN = 1
			...
			if @brandID = 0
				Begin
					...
					from #TempSubscription t 
						join SubscriptionDetails sd with (nolock) on sd.SubscriptionID  = t.subsID 
						join mastercodesheet mc with (nolock) on mc.masterID = sd.masterID 
				...
				Begin
					...
					select distinct subscriptionID, mg.DisplayName,  mc.MasterValue
					from #TempSubscription t 
						join vw_BrandConsensus v on v.SubscriptionID  = t.subsID 
						join mastercodesheet mc with (nolock) on mc.masterID = v.masterID 
						join mastergroups mg with (nolock) on mg.mastergroupID = mc.mastergroupID 
						join #TempMasterGroups tg with (nolock) on tg.MasterGroupColumnReference = mg.ColumnReference
					...
					from #TempSubscription t 
						...
						join mastercodesheet mc with (nolock) on mc.masterID = v.masterID 
						join mastergroups mg with (nolock) on mg.mastergroupID = mc.mastergroupID 
						join #TempMasterGroupsDesc tg with (nolock) on tg.MasterGroupColumnReference = mg.ColumnReference 
					where (v.BrandID = @brandID)
				end		
	...
	ELSE
		BEGIN
			if(LEN(@SubExtMapperColsAliased) > 0)
				begin
					SET @query =	'select ' + isnull(@scolumns,'') + isnull(@SubExtMapperColsAliased,'') + isnull(@custcolumns,'') + '
