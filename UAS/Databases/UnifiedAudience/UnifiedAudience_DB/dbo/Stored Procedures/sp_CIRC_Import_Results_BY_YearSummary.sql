CREATE Proc [dbo].[sp_CIRC_Import_Results_BY_YearSummary]
(
...
BEGIN
	...
	select t.pubID, t.pubcode, t.pubsubscriptionID, t.sequenceID, t.FirstName, t.LastName, 
		...
		case 
			when historyQDAte IS null and circ.QSourceDate is null then NULL
			when historyQDAte IS null and circ.QSourceDate is not null then circ.SQSource
			when historyQDAte IS not null and circ.QSourceDate is null then historyQsource
			when historyQDAte >= circ.QSourceDate then historyQsource else circ.SQSource end 
		as	Previous_QSource,
		
		case 
			when historyQDAte IS null and circ.QSourceDate is null then NULL
			when historyQDAte IS null and circ.QSourceDate is not null then circ.QSourceDate
			when historyQDAte IS not null and circ.QSourceDate is null then historyQDAte
			when historyQDAte >= circ.QSourceDate then historyQDAte else circ.QSourceDate end 
		as	Previous_Qdate,
		
		case 
			when historyQDAte IS null and circ.QSourceDate is null then NULL
			when historyQDAte IS null and circ.QSourceDate is not null then 
			...
			when historyQDAte IS not null and circ.QSourceDate is null then 
			...
			when historyQDAte >= circ.QSourceDate then 
		...
		case 
			when historyQDAte IS null and circ.QSourceDate is null then NULL
			when historyQDAte IS null and circ.QSourceDate is not null then circ.CategoryCodeValue
			when historyQDAte IS not null and circ.QSourceDate is null then history_CAT
			when historyQDAte >= circ.QSourceDate then history_CAT else circ.CategoryCodeValue end 
		as	Previous_CAT,
		
		case 
			when historyQDAte IS null and circ.QSourceDate is null then NULL
			when historyQDAte IS null and circ.QSourceDate is not null then circ.TransactionCodeValue
			when historyQDAte IS not null and circ.QSourceDate is null then history_XACT
			when historyQDAte >= circ.QSourceDate then history_XACT else circ.TransactionCodeValue end 
		as	Previous_XACT,
		
		case 
			when historyQDAte IS null and circ.QSourceDate is null then NULL
			when historyQDAte IS null and circ.QSourceDate is not null then Circ.TransactionCodeTypeName
			when historyQDAte IS not null and circ.QSourceDate is null then history_TransactionCodeTypeName
			when historyQDAte >= circ.QSourceDate then history_TransactionCodeTypeName else Circ.TransactionCodeTypeName end 
		as	Previous_TransactionCodeTypeName
	into #tmp_Results 	
	from #tmp_ProcessedRecords T
		...
		(
			...
			(	
				...
				ROW_NUMBER() over (partition by hs.sequenceID order by hs.DateCreated desc, hs.QualificationDate desc) as rown
	...
	select Isnull(Previous_Year, 'New') as 'ChangedFrom', 
		YEAR,
	...
	--select 
	--		Isnull(Previous_Year, 'New') 'ChangedFrom', 
	--		YEAR, 
