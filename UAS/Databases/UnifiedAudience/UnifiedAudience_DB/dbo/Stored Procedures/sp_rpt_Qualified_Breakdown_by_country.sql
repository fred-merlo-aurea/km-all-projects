...
Begin
	...
	(			
			...
			From 
					...
					#subscriptionIDs ts on ts.SubscriptionID = s.SubscriptionID join  
					UAD_Lookup..CategoryCode cc with (nolock) on s.CategoryID = cc.CategoryCodeID join      
					UAD_Lookup..CategoryCodeType cct with (nolock) on cct.CategoryCodeTypeID = cc.CategoryCodeTypeID join            
					UAD_Lookup..[TransactionCode] t  with (nolock) on s.TransactionID = t.TransactionCodeID  left outer join
					UAD_Lookup..country ct with (nolock) on  s.CountryID = ct.CountryID
			group by 
