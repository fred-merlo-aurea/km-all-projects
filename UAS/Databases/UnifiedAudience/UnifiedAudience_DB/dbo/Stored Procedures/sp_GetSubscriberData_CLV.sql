CREATE PROCEDURE [dbo].[sp_GetSubscriberData_CLV]
@Queries XML,
@StandardColumns XML,
...
Begin 
	...
	CREATE TABLE #TempSubscription (ID int identity(1,1), subsID int, CGRP_No uniqueidentifier);
		...
		BEGIN
			...
			if @brandID = 0
				Begin
					...
					select distinct subsID, mg.DisplayName,  mc.masterValue
					from #TempSubscription t 
						join CompanyGroupDetails cgd with (nolock) on t.CGRP_No  = cgd.CGRP_No 
						join mastercodesheet mc with (nolock) on mc.masterID = cgd.masterID 
					...
					select distinct subsID, mg.DisplayName + '_Description',  mc.MasterValue,  
				...
				Begin
					Insert into #TempSubscriptionDetails
					select distinct t.subsID, mg.DisplayName,  mc.MasterValue
					from #TempSubscription t 
						join vw_BrandConsensus_CLV v on v.CGRP_No  = t.CGRP_No 
					...
					Insert into #TempSubscriptionDetailsDesc
					select distinct t.subsID, mg.DisplayName + '_Description',  mc.MasterValue,  
							(Case  when tg.CasetoConvert = 'uppercase' then upper(cast(mc.MasterDesc as  varchar(100))) 
							when tg.CasetoConvert = 'lowercase' then lower(cast(mc.MasterDesc as  varchar(100))) 
							when tg.CasetoConvert = 'propercase' then master.dbo.fn_title_case(cast(mc.MasterDesc as  varchar(100))) 
							else cast(mc.MasterDesc as  varchar(100)) end)
