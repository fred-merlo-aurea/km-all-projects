CREATE Proc [dbo].[rpt_GetSubscriptionIDs_Copies_From_Filter]
(  
...
BEGIN
	...
	set @executeString += 'Select	DISTINCT ps.SubscriptionID as Count, ps.Copies FROM	PubSubscriptions ps with(nolock) '
			...
			BEGIN	
				if len(@Year) > 0 and CHARINDEX('0', @year) = 0
					Begin
						...
						select @tempStartDate = ltrim(rtrim(YearStartDate)) , @tempEndDate = ltrim(rtrim(YearEndDate)) from Pubs with(nolock) where PubID = @PublicationID		
					...
					BEGIN
						set @executeString += (CASE WHEN LEN(ISNULL(@regResponse,'')) > 0 THEN ' AND '
												...
												'(ps.SubscriptionID not in (SELECT DISTINCT pd.SubscriptionID from PubSubscriptionDetail pd 
													JOIN PubSubscriptions p ON p.PubSubscriptionID = pd.PubSubscriptionID 
													JOIN CodeSheet c ON pd.CodeSheetID = c.CodeSheetID
													WHERE p.PubID = ' + CONVERT(varchar(50),@PublicationID) + ' AND c.ResponseGroupID in (' + @noResponse + ')))'
					END
			END
