CREATE PROCEDURE [dbo].[o_BatchHistoryDetail_Select_BatchID]
@BatchID int,
...
BEGIN
	...
	SET NOCOUNT ON
	----Start Test Data--------------------------------------
	--DECLARE @ClientName varchar(100) = 'Name', @BatchID int = 1
	----End Test Data----------------------------------------
		...
		Select UserLogID, UserLogTypeID, [Object], FromObjectValues, ToObjectValues, DateCreated
			...
			(
				...
				from Batch b With(NoLock)
					...
					where b.BatchID = @BatchID and htul.UserLogID is not null		
			)
	END
	else
	BEGIN
