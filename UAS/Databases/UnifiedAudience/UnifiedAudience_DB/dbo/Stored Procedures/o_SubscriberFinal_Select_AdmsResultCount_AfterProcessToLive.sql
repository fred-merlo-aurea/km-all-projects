CREATE PROCEDURE o_SubscriberFinal_Select_AdmsResultCount_AfterProcessToLive
...
as
	begin
		...
		where sf.ProcessCode = @processCode and sf.Ignore = 'false') as 'FinalProfileCount',
		...
		where sf.ProcessCode = @processCode and sf.Ignore = 'false') as 'FinalDemoCount',
		--
		(select count(sf.SFRecordIdentifier)
		from SubscriberFinal sf with(nolock)
