...
BEGIN
	...
	select @publicationID = publicationID, @row = [ROW]  
	...
	if @row = 'DEMO7'
		Begin
			select d.deliverabilitycode as responseID, case when DeliverabilityCode = 'A' then 'A. Print Edition' when DeliverabilityCode = 'B' then 'B. Digital Edition' when DeliverabilityCode = 'C' then 'C. Both' end as responsevalue, SUM(SF.COPIES) as counts 
			From Subscription s 
				join #SubscriptionID sf on sf.SubscriptionID = s.SubscriptionID 
				join Deliverability d on d.DeliverabilityID  = s.DeliverabilityID
			group by d.deliverabilitycode, d.deliverabilityName
