CREATE PROCEDURE [dbo].[job_ListSource_Other]
...
BEGIN
	...
	--	and cs.PubQSourceID in (Select C.CodeId from UAD_Lookup..Code C join UAD_Lookup..CodeType CT on C.CodeTypeId = CT.CodeTypeId 
	--		where CT.CodeTypeName = 'Qualification Source' and C.CodeValue in ('A','B','C','D','E','F','G','H','Q','R','S')))
	--and cs.PubTransactionID not in (Select tc.TransactionCodeID
	--						   From UAD_Lookup..Action a join UAD_Lookup..CategoryCode cc on a.CategoryCodeID = cc.CategoryCodeID
