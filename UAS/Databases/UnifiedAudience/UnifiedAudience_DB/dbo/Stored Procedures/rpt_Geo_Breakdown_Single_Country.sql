CREATE PROCEDURE [dbo].[rpt_Geo_Breakdown_Single_Country]
(    
	@ReportID int,
	@ProductID int, 
...
BEGIN
	...
	set @country = (SELECT ROW FROM Reports with(nolock) WHERE ReportID = @ReportID)
	...
	Begin
		select	state as 'State/Province', 
  		...
  		from
			...
			where PubID = @ProductID and C.CategoryCodeTypeID <> 5 and s.CountryID = (SELECT CountryID FROM UAD_Lookup..Country WHERE ShortName LIKE @country)
