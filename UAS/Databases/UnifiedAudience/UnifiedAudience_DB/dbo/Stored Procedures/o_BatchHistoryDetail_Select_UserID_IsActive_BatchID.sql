CREATE PROCEDURE [dbo].[o_BatchHistoryDetail_Select_UserID_IsActive_BatchID]
@UserID int,
@IsActive bit,
...
BEGIN
	...
	SET NOCOUNT ON
	----Start Test Data--------------------------------------
	--DECLARE @ClientName varchar(100) = 'Name', @UserID int = 1, @IsActive bit = 'false', @BatchID int = 1
	----End Test Data----------------------------------------
		...
		Select UserLogID, UserLogTypeID, [Object], FromObjectValues, ToObjectValues, DateCreated
			...
			(
				...
				from Batch b With(NoLock)
					...
					where b.UserID = @UserID and b.IsActive = @IsActive and b.BatchID = @BatchID and htul.UserLogID is not null		
			)
	END
	else
	BEGIN
	...
	WHERE b.UserID = @UserID AND b.IsActive = @IsActive and htul.UserLogID != h.HistorySubscriptionID and b.BatchID = @BatchID
