CREATE Proc [dbo].[sp_CIRC_Import_Results_By_CATTRANS_Summary]
(
	...
	from #tmp_ProcessedRecords T
		...
		(
			...
			(	
				...
				ROW_NUMBER() over (partition by hs.sequenceID order by (case when hs.DateCreated > isnull(hs.dateupdated,'1-1-1900') then hs.DateCreated else hs.dateupdated end) desc) as rown
