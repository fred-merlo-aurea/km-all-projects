CREATE PROCEDURE [dbo].[o_SaveArchivePubSubscriptions_AdHocs]
...
BEGIN
	...
	END
	IF NOT EXISTS (SELECT TOP 1 * FROM IssueArchivePubSubscriptionsExtension WHERE IssueArchiveSubscriptionID = @IssueArchiveSubscriptionID)
		BEGIN
			INSERT INTO IssueArchivePubSubscriptionsExtension (IssueArchiveSubscriptionID, DateCreated, CreatedByUserID)
			VALUES(@IssueArchiveSubscriptionID, GETDATE(), 1)
		END
	DECLARE @PubCodeID int,
	@PubStandardField varchar(255),
	@PubCustomField varchar(255),
	@PubValue varchar(255)
	DECLARE @updateClause varchar(max) = 'UPDATE ArchivePubSubscriptionsExtension SET '

	--SET @PubID = 22
	--SET @PubSubscriptionID = 23525646

	CREATE TABLE #tblPubAdhoc (IssueArchiveSubscriptionID int, AdhocValue varchar(max))

	CREATE INDEX IDX_tblPubAdhoc_SubscriptionID ON #tblPubAdhoc(IssueArchiveSubscriptionID)
	...
	SET @updateClause += ' WHERE IssueArchiveSubscriptionID = ' + CONVERT(varchar(25), @IssueArchiveSubscriptionID)
	EXEC (@updateClause)
