create procedure o_Get_FileMappingColumnValues_ClientId_PubId
@clientId int = 0,
@pubId int
as
	begin
		...
		insert into @ruleField (rfId,DataTable,TablePrefix,ColumnName,DataType,IsDemographic,IsDemographicOther,IsMultiSelect,ClientId,UxControl) exec o_Get_FileMappingColumns_ClientId_PubId @clientId, @pubId
		...
		where x.DataTable='PubSubscriptions' and x.ColumnName='PUBCODE' and p.PubId = @pubId
		union
		--****** ResponseGroups *********
		select distinct 
					x.rfId as 'Id',
		...
		where x.DataTable='ResponseGroups' and rg.PubId = @pubId
	end
go
