			...
			if (@OnlyCount = 1)
				begin
					...
					From Subscription s 
						...
						join Subscriber sb on s.SubscriberID = sb.subscriberID 
						join Action a on a.ActionID = s.ActionID_Current 
						join CategoryCode cc on cc.CategoryCodeID  = a.CategoryCodeID 
						join CategoryCodeType cct on cct.CategoryCodeTypeID = cc.CategoryCodeTypeID 
						join TransactionCode tc on tc.TransactionCodeID = a.TransactionCodeID
					where PublicationID = @PublicationID			 
				end
			else
				begin
