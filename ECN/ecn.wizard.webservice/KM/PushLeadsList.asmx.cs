...
namespace ecn.wizard.webservice.KM {
	...
	/// </summary>
	[WebService(
		 Namespace="http://wizard.ecn5.com/webservice/KM/PushLeadsList.asmx", 
				...
				foreach(DataRow currentRow in currentDataSetRow){
					...
					if (null == currentEmail) {
						try{
							...
							try{
								bd=DateTime.Parse("");
							}catch{ bd = sqldatenull;}
							try{
								uedt1=DateTime.Parse("");
							}catch{ uedt1 = sqldatenull; }
							try{
								uedt2=DateTime.Parse("");
							}catch{ uedt2 = sqldatenull; }
							...
							InsertCommand.Parameters.Add ("@notes",SqlDbType.Text).Value = "Subscription through wizard WebService. DateAdded: "+DateTime.Now.ToString(); 
