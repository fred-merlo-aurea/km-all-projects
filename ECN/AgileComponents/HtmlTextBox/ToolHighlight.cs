...
{
	/// <summary>
	/// Represents a <see cref="ToolHighlight"/> object.
	/// </summary>
    [ToolboxItem(false)]
	public class ToolHighlight : ToolColorPickerBase
	{
		/// <summary>
		/// Initializes a new instance of the <see cref="ToolHighlight"/> class.
		/// </summary>
		public ToolHighlight() : base()
		{
			_Init(string.Empty);
		}
		...
		public ToolHighlight(string id) : base(id)
		{
			_Init(id);
		}
		...
		{
			if (id == string.Empty)
				this.ID = "_toolHighlight" + Editor.indexTools++;
			else
				this.ID = id;
			//this.ClientColorSelected =  "HTB_SetColorEditor('$EDITOR_ID$','b',HTB_GetSelectedColor('$CLIENT_ID$'))"; 
			this.ToolTip = "Highlight Color";
            this.Text = "<img src=$IMAGESDIRECTORY$highlight_off.gif>";
		}
        ...
        public override void RenderDesign(HtmlTextWriter output)
        {
            ...
            {
                this.Text = string.Format("<img src='{0}'>", Page.ClientScript.GetWebResourceUrl(this.GetType(), "ActiveUp.WebControls._resources.Images.highlight_off.gif"));                
            }
#endif
		...
		{
#if (!FX1_1)
            if (Text == "<img src=$IMAGESDIRECTORY$highlight_off.gif>")
                this.Text = string.Format("<img src={0}>", Page.ClientScript.GetWebResourceUrl(this.GetType(), "ActiveUp.WebControls._resources.Images.highlight_off.gif"));
#endif
			...
			if (editor != null)
			{
				...
				this.ClientColorSelected =  string.Format("HTB_SetColorEditor('{0}','b',HTB_GetSelectedColor('{1}'))",editor.ClientID,this.ClientID); 
			}
		}
		...
		protected override void Render(HtmlTextWriter output)
		{
...
#if (FX1_1)
            this.Text =  string.Format("<img src={0}>",Utils.ConvertToImageDir(editor.IconsDirectory,"highlight_off.gif"));
#endif
			...
			output.Write("{\n");
			/*output.Write(string.Format("ATB_createPopup(\"{0}_CustomColors\",0, 0, {2}, {3}, \"Custom Colors\", HTB_BuildColorTable(\"{4}\",\"{0}\",\"HTB_SetSelectedColor('{0}',$color$);ATB_hidePopup('{0}_CustomColors');{1}\",false,\"b\"),\"#DDDDDD\",\"#DDDDDD\",\"Outset\",\"2\",\"#4F6DA5\",\"#808080\",\"#FFFFFF\",\"NotSet\",\"0\",\"#FFFFFF\",\"#DDDDDD\",\"#FFFFFF\",\"NotSet\",\"0\", \"#808080\",\"True\",\"False\",undefined,\"True\")",ClientID,ClientColorSelected,base.WidthPopup,base.HeightPopup,editor.ClientID,EnableSsl));	
