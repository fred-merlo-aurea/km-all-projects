...
namespace ecn.showcare.webservice.Objects {
	...
	public class User {
		...
		public bool Exists {
			get {
				string sql = string.Format(@"if exists(select * from Users where userName = '{0}' and CustomerID = {1})
select 1
else
select 0", this.UserName, this.Customer.ID);				
				return Convert.ToBoolean(DataFunctions.ExecuteScalar(sql));
			}
		}

		public int getUserID() {
			string sql = string.Format(@"SELECT UserID FROM Users WHERE UserName = '{0}' and CustomerID = {1}", this.UserName, this.Customer.ID);				
			int uID = 0;
			try{
