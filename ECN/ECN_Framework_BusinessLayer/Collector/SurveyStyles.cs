...
namespace ECN_Framework_BusinessLayer.Collector
{
    ...
    {
        public static void Save(ECN_Framework_Entities.Collector.SurveyStyles sStyle, KMPlatform.Entity.User user)
        {
            using (TransactionScope scope = new TransactionScope())
            {
                ECN_Framework_DataLayer.Collector.SurveyStyles.Save(sStyle);
                scope.Complete();
            }
        }

        public static ECN_Framework_Entities.Collector.SurveyStyles GetBySurveyID(int SurveyID, KMPlatform.Entity.User user)
        {
            ECN_Framework_Entities.Collector.SurveyStyles item = new ECN_Framework_Entities.Collector.SurveyStyles();
            using (TransactionScope scope = new TransactionScope(TransactionScopeOption.Suppress))
            {
                item = ECN_Framework_DataLayer.Collector.SurveyStyles.GetBySurveyID(SurveyID, user.UserID);
                scope.Complete();
            }
            return item;
        }

        public static string RenderStyle(ECN_Framework_Entities.Collector.SurveyStyles sStyle)
        {
            char[] delimiter = { ' ' };
