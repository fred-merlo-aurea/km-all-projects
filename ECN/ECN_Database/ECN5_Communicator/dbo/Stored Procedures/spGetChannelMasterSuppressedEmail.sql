CREATE PROCEDURE [dbo].[spGetChannelMasterSuppressedEmail]   
(  
	@BaseChannelID int,   
	@PageNo int,  
	@PageSize int,
...
Begin  
	...
	declare @CMS TABLE (RowNum int, CMSID int , Emailaddress varchar(100), DateAdded datetime)		
	...
	begin
		...
		WHERE BaseChannelID = @BaseChannelID and EmailAddress like '%' + @EmailSearchString + '%'
	End
	Else
	Begin
		INSERT INTO @CMS 
	...
	SELECT	CMSID,  EmailAddress, DateAdded 
	FROM @CMS
	WHERE RowNum >= @FirstRec 
	AND RowNum <= @LastRec
