CREATE PROCEDURE [dbo].[spgetBlastEmailsListUsingThreshold](
	@CustomerID int,        	
	@BlastID int,
	@GroupID int,        
	@FilterID int,        
...
BEGIN        
	...
	set @filter = replace(@filter,'emailaddress','emails.emailaddress')
 	...
 	Begin
 		...
 		SELECT 
			@customerProduct = count(p.productname) 
		FROM 
		...
		IF @customerProduct > 0 AND @thresholdlimit IS NOT NULL AND @thresholdlimit > 0
			BEGIN
	...
	End
 	...
 	begin        
		...
		Begin   
			if (not ((@CustomerID = 1041 and @emailsubject like 'Return Your Textbooks%' and @EmailFrom = 'textbookdept@nebook.com') or @CustomerID= 2808))
			Begin	
				insert into #blacklist        
	...
	END
  	...
  	if not exists(select * from @gdf)  or (@OnlyCounts = 1 and len(@Filter) = 0)
  	Begin
		if @OnlyCounts = 0		 
		Begin			
			...
			Begin
				select @selectslotstr = dbo.fn_DCSelectString(@layoutID) 
					...
					exec ( @selectslotstr + ' ( select top ' +  @topcount + ' Emails.EmailID,'''+ @BlastID +''' as BlastID, '''+@mailRoute+''' as mailRoute,  Emails.EmailAddress, CustomerID, ' + @emailcolumns + 
								' groupID, FormatTypeCode, SubscribeTypeCode,  emailgroups.CreatedOn, emailgroups.LastChanged, ' +        
								'''eid='' + Convert(varchar,Emails.EmailID)+''&bid=' +  @BlastID + ''' as ConversionTrkCDE, ' +        
								'''bounce_''+Convert(varchar,Emails.EmailID)+''-''+''' + @BlastID_and_BounceDomain + ''' as BounceAddress, '''+ @CurrDate + ''' as CurrDate ' +        
								' into #tmpEmailList2 from  Emails join EmailGroups on EmailGroups.EmailID = Emails.EmailID ' +    
								' left outer join 	ChannelMasterSuppressionList cms on emails.emailaddress = cms.emailaddress and cms.basechannelID = '  +  @basechannelID  + 
								' left outer join   GlobalMasterSuppressionList gms on emails.emailaddress = gms.emailaddress ' + 
								' left outer join #blacklist blist on blist.emailID = Emails.emailID ' + 
								' where Emails.CustomerID = ' +  @CustomerID  + ' and Emails.EmailAddress like ''%@%.%'' and EmailGroups.SubscribeTypeCode = ''S''' +        
	...
	DECLARE @StandAloneUDFs VARCHAR(2000),
		...
		--SELECT  @StandAloneUDFIDs = STUFF(( SELECT DISTINCT ',' + convert(varchar(10),GroupDatafieldsID) FROM groupdatafields WHERE GroupID = @GroupID AND DatafieldSetID is null ORDER BY ',' + convert(varchar(10),GroupDatafieldsID) FOR XML PATH('') ), 1, 1, '') 
		...
		--SELECT  @TransactionalUDFIDs = STUFF(( SELECT DISTINCT ',' + convert(varchar(10),GroupDatafieldsID) FROM groupdatafields WHERE GroupID = @GroupID AND DatafieldSetID > 0 ORDER BY ',' + convert(varchar(10),GroupDatafieldsID) FOR XML PATH('') ), 1, 1, '') 
	
		SELECT  @StandAloneUDFs = STUFF(( SELECT DISTINCT '],[' + g.ShortName FROM groupdatafields g join @gdf tg on g.GroupDatafieldsID = tg.GID 
		WHERE DatafieldSetID is null ORDER BY '],[' + g.ShortName FOR XML PATH('') ), 1, 2, '') + ']'
		...
		WHERE DatafieldSetID is null ORDER BY ',' + convert(varchar(10),GroupDatafieldsID) FOR XML PATH('') ), 1, 1, '') 
		...
		WHERE isnull(DatafieldSetID,0) > 0 ORDER BY '],[' + g.ShortName FOR XML PATH('') ), 1, 2, '') + ']'       
		...
		WHERE isnull(DatafieldSetID,0) > 0 ORDER BY ',' + convert(varchar(10),GroupDatafieldsID) FOR XML PATH('') ), 1, 1, '') 		
		...
		Begin
			...
			Begin
				...
				Begin
					...
					' into #tmpEmailList3 from Emails ' +       
				...
				Begin
					...
					'''eid='' + Convert(varchar,Emails.EmailID) +''&bid=' + @BlastID + ''' as ConversionTrkCDE, emailgroups.CreatedOn, emailgroups.LastChanged, ' +      
					'''bounce_''+ Convert(varchar,Emails.EmailID) +''-''+'''+@BlastID_and_BounceDomain+''' as BounceAddress, '''+ @CurrDate + ''' as CurrDate, ' + @emailcolumns + @sColumns + @tColumns +     
					'  into #tmpEmailList4  from Emails ' +       
			...
			Begin
				select @selectslotstr = dbo.fn_DCSelectString(@layoutID) 
				exec (@selectslotstr + ' ( select top ' + @topcount + ' Emails.EmailID,' + @BlastID + ' as BlastID, '''+@mailRoute+''' as mailRoute, Emails.EmailAddress, Emails.CustomerID, GroupID, FormatTypeCode, SubscribeTypeCode, ' +      
				'''eid='' + Convert(varchar,Emails.EmailID) +''&bid= '+ @BlastID + ''' as ConversionTrkCDE, emailgroups.CreatedOn, emailgroups.LastChanged, ' +      
				'''bounce_''+ Convert(varchar,Emails.EmailID) +''-''+'''+@BlastID_and_BounceDomain+''' as BounceAddress,  '''+ @CurrDate + ''' as CurrDate, ' +  @emailcolumns + @sColumns + @tColumns +   
				'  into #tmpEmailList5 from Emails  ' +       
				' join EmailGroups  on EmailGroups.EmailID = Emails.EmailID ' +        
				' left outer join #blacklist blist on blist.emailID = Emails.emailID '  +@standAloneQuery + @TransactionalQuery +   
				' left outer join 	ChannelMasterSuppressionList cms on emails.emailaddress = cms.emailaddress and cms.basechannelID = '  + @basechannelID + 
				' left outer join 	GlobalMasterSuppressionList gms on emails.emailaddress = gms.emailaddress ' + 
