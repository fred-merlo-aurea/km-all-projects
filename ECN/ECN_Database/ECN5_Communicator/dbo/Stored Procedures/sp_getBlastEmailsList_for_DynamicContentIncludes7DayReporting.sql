...
--select top 100 * from [BLAST] where customerID = 125 order by blastID desc
--set @CustomerID = 2486
--set 	@BlastID = 142953
--set 	@GroupID = 19631
--set 	@FilterID = '0'
...
BEGIN        
	...
	Begin
		...
		insert into #blacklist
		select emailID, 'Sample' from emailactivitylog   WITH (NOLOCK) 
 	...
 	begin        
		...
		Begin   
			...
			Begin	
				...
				WHERE         
					BlastID in (SELECT BlastID FROM [BLAST]   WITH (NOLOCK) WHERE LayoutID = @LayoutID) AND         
	...
	BEGIN
		set @EmailString = @EmailString + ' and Emails.EmailID in (SELECT EmailID from EmailActivityLog E WHERE BlastID in (' + @refBlastID + ') and ActionTypeCode = ''send'' ) '
	END
		...
		Begin
			...
			Begin
				...
				Begin
					...
					' into #tmpEmailList3 from Emails left outer join ' +       
						 '(select InnerTable1.EmailID as tmp_EmailID, InnerTable1.EntryID, ' + @Col1 + ' from  ' +         
							   '(select e.EmailID, E.EntryID, ' + @Col2 + ' from #E E left outer join #blacklist bl on bl.emailID = e.emailID where bl.emailID is null) as InnerTable1 Group by InnerTable1.EmailID, InnerTable1.EntryID ' +         
					...
					where	t1.emailaddress is null and
							 InBlackList = 0 and
							t.emailID in (select distinct top ' + @topcount + ' emailID from #tmpEmailList3) 
					order by t.emailID
				...
				Begin
					...
					where	t1.emailaddress is null and
							 InBlackList = 0 and
							t.emailID in (select distinct top ' + @topcount + ' emailID from #tmpEmailList4) 
					order by t.emailID
			...
			Begin
				...
				' and EmailGroups.SubscribeTypeCode = ''S'' and blist.emailID is null and cms.emailaddress is null ' + @Filter + ' ' +  @EmailString + ') inn2 order by inn2.EmailID 
					...
					where	t1.emailaddress is null and
							 InBlackList = 0 and
							t.emailID in (select distinct top ' + @topcount + ' emailID from #tmpEmailList5) 
					order by t.emailID
