CREATE PROCEDURE [dbo].[e_Email_UpdateEmail_Customer]
...
AS
	...
	Select @OldEmailID = EmailID from Emails e with(nolock) where e.CustomerID = @CustomerID and e.EmailAddress = @OldEmail

if exists(Select top 1 EmailID from Emails e with(nolock) where e.CustomerID = @CustomerID and e.EmailAddress = @NewEmail)
BEGIN
	--New email already exists so we're updating profile fields from old email if they don't already have a value
			
	Select @NewEmailIDToUpdate = EmailID from Emails e with(nolock) where e.EmailAddress = @NewEmail and e.CustomerID = @CustomerID
...
BEGIN
	...
	where CustomerID = @CustomerID and EmailID = @OldEmailID
...
where g.CustomerID = @CustomerID and ISNULL(g.MasterSupression,0) = 1
...
where eg.EmailID = @OldEmailID  and eg.GroupID <> @CustMSGroupID and g.CustomerID = @CustomerID
