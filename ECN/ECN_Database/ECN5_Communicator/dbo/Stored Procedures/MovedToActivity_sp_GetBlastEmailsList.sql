CREATE procedure [dbo].[MovedToActivity_sp_GetBlastEmailsList]
(	
	@CustomerID int,        	
	@BlastID int,
	@GroupID int,        
...
*/
BEGIN    
	INSERT INTO ECN_ACTIVITY..OldProcsInUse (ProcName, DateUsed) VALUES ('sp_GetBlastEmailsList', GETDATE())    
	set NOCOUNT ON        
	...
	select @DynamicFromName = DynamicFromName, @DynamicFromEmail = DynamicFromEmail, @DynamicReplyToEmail = DynamicReplyToEmail from [BLAST] where blastID = @BlastID
	...
	End
 	...
 	Begin       
		...
		exec('insert into #tempB select EmailID from EmailActivityLog el where BlastID in (' + @refBlastID +') and ActionTypeCode = ''send'' ') 
...
select top ' + @topcount + ' Emails.EmailID , ' + @BlastID + ' as BlastID, LTRIM(RTRIM(emails.EmailAddress)) AS EmailAddress, Emails.CustomerID, GroupID, FormatTypeCode, SubscribeTypeCode, ' +      
			...
			' from Emails left outer join #tmpUDF as InnerTable2 on Emails.EmailID = InnerTable2.tmp_EmailID join ' +        
