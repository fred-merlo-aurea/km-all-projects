CREATE  PROCEDURE [dbo].[MovedToActivity_sp_downloadBlastEmailsWithSuppressed](
	@blastID varchar(10), 
...
AS
BEGIN 
	INSERT INTO ECN_ACTIVITY..OldProcsInUse (ProcName, DateUsed) VALUES ('sp_downloadBlastEmailsWithSuppressed', GETDATE())
	--set @blastID = 433151
	--set @ReportType = 'send'
	--set @FilterType = ''
	--set @ISP = ''
	...
	Begin
		...
		begin
			...
			SELECT  eal.EmailID, eal.ActionDate, '', ''
			FROM	EmailActivityLog eal JOIN Emails e ON eal.EMailID=e.EMailID  
			WHERE	BlastID=@blastID AND ActionTypeCode = @ReportType and e.emailaddress like '%' + @ISP 
		end
		else
		begin
			insert into #tempA  
			SELECT  eal.EmailID, eal.ActionDate, '', ''
			FROM	EmailActivityLog eal JOIN Emails e ON eal.EMailID=e.EMailID  
			WHERE	BlastID=@blastID AND ActionTypeCode = @ReportType and eal.ActionValue = @FilterType and e.emailaddress like '%' + @ISP 
		end
