...
Begin        
	...
	Begin
		...
		FROM  
				EmailActivityLog eal with (nolock) join dbo.[fn_Blast_Report_Filter_By_UDF](@blastID,@UDFname,@UDFdata ) t on eal.emailID = t.emailID
		WHERE 
				eal.BlastID = @blastID 
		...
		FROM (        
				...
				FROM   EmailActivityLog eal with (nolock) join dbo.[fn_Blast_Report_Filter_By_UDF](@blastID,@UDFname,@UDFdata ) t on eal.emailID = t.emailID        
