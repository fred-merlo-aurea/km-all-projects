...
IF EXISTS (SELECT 1 FROM Sysobjects where name = 'getEnewsDataForADMS_by_GroupID')
DROP Proc getEnewsDataForADMS_by_GroupID
GO
...
CREATE PROCEDURE [dbo].[getEnewsDataForADMS_by_GroupID]
(
...
as
Begin
	-- EXECUTE getEnewsDataForADMS_by_GroupID 'WATT', 0 --1
	-- EXECUTE getEnewsDataForADMS_by_GroupID @ClientName = 'WATT', @GroupID = 248637, @FullExport = 1

	-- EXECUTE getEnewsDataForADMS 'CANON', 0, -103, '3/9/2016'
	-- EXECUTE getEnewsDataForADMS 'EHPublishing',   0, -103, '2/27/2017'
	...
	-- EXECUTE getEnewsDataForADMS_by_groupID @ClientName = 'BABCOX', @GroupID = 334946, @FullExport = 0, @DaysOld = -1, @EndDate = '2/27/2017'
	-- EXECUTE getEnewsDataForADMS_by_groupID @ClientName = 'Pennwell', @GroupID = 0, @FullExport = 0, @DaysOld = -1, @EndDate = '2/27/2017', @GroupList = '275647,275674,278274,278275'
	-- Select * from ecn_temp.dbo.MAF_CANON_Enews where lastchanged > '3/10/2016'
	-- Select * from ecn_temp.dbo.MAF_MacFadden_Enews 
	-- Select * from ecn_temp.dbo.MAF_EHPublishing_Enews 
	-- Select * from ecn_temp.dbo.MAF_WATT_Enews 
	-- Select * from ecn_temp.dbo.MAF_Pennwell_Enews 
	-- Select * from ecn_temp.dbo.MAF_babcox_Enews 
...
DECLARE @SQL varchar(max)
		SELECT @SQL = 'insert into #groups select distinct groupID, Pubcode, EnewsTransformGrouptoPubCode from dbo.UAD_ENEWS_SYNC where client = ''' + @clientName + ''' and  GroupID in ('  + @groupList + ') and IsActive = 1'
		Execute (@SQL)	
	END
	
	if (Select COUNT(*) from #groups) = 0
	BEGIN
		print 'ERROR - Client is not setup with groups or Selected GroupID in UAD_ENEWS_SYNC Table'
	END
				...
				SELECT *
				 ...
				 ( 
					...
					where 
							...
							and gdf.ShortName in (select fieldname from ecn_temp..EnewsExportFields)
				 ) u
				 PIVOT
				 (
				 MAX (DataValue)
	...
	Begin
		...
		(
			SELECT *
			 FROM
			 (
				SELECT gdf.groupID As TUDF_GID, edv.emailID as tmp_EmailID1, ' +
				--edv.entryID, 
				'gdf.ShortName, REPLACE(DataValue,CHAR(9),CHAR(32)) AS DataValue
				from	EmailDataValues edv with (NOLOCK)  join  
						Groupdatafields gdf with (NOLOCK) on edv.GroupDatafieldsID = gdf.GroupDatafieldsID
				where 
						ISNULL(gdf.IsDeleted,0) = 0 and gdf.GroupID in (select grpID from #groups)  and datafieldsetID > 0 and ISNULL(edv.datavalue, '''') <> '''' '
						
			if @clientName <> 'UPI'
				set @TransactionalQuery += ' and gdf.SHORTNAME  != ''TOPICS'' '
						
			 set @TransactionalQuery += ') u
			 PIVOT
			 (
			 MAX (DataValue)
