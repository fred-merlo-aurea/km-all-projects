CREATE  PROCEDURE [dbo].[MovedToActivity_sp_filteremails_with_smartsegment_with_blacklist](
	@GroupID int,        
	@CustomerID int,        
	@FilterID int,        
	@Filter varchar(8000),        
...
AS
BEGIN	
	INSERT INTO ECN_ACTIVITY..OldProcsInUse (ProcName, DateUsed) VALUES ('sp_filteremails_with_smartsegment_with_blacklist', GETDATE())
	--set @GroupID = 52958
	--set @CustomerID = 3137
	--set @FilterID = 27919;
	--set @Filter = ''
	...
	WHERE 	g.CustomerID = @CustomerID AND g.MasterSupression=1        
 	...
 	begin        
		...
		select @emailsubject = EmailSubject from [BLAST] where blastID = @BlastID
