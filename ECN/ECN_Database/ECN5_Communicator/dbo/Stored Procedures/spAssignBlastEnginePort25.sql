CREATE proc [dbo].[spAssignBlastEnginePort25]
as
BEGIN
...
c.ActiveFlag = 'Y' and 
	...
	(b.BlastType='html' OR b.BlastType='text' OR b.BlastType='sample' OR b.BlastType='champion' OR b.BlastType='personalization') 
...
BEGIN 
	...
	IF	NOT EXISTS (SELECT TOP 1 CampaignItemBlastID FROM CampaignItemBlast WHERE BlastID = @CurrentBlast AND IsDeleted = 0) AND 
	...
	BEGIN	
		...
		BEGIN
			...
			WHERE IsActive = 1 and IsPort25 = 1 and ISNULL(IsDedicatedEngine,0) = 1 and Name like 'ECNBlastEngine_Test%'
		...
		BEGIN
			...
			BEGIN
				SELECT TOP 1 @BlastEngineID = BlastEngineID 
				FROM [BlastEngines] WITH (NOLOCK)
				WHERE IsActive = 1 and IsPort25 = 1 and ISNULL(IsDedicatedEngine,0) = 0 
					AND BlastEngineID NOT IN (SELECT DISTINCT BlastEngineID FROM [BLAST] WITH (NOLOCK) WHERE StatusCode = 'active' OR (BlastEngineID IS NOT NULL AND StatusCode = 'pending'))
				ORDER BY BlastEngineID ASC	
			END
		END
...
--Exec spAssignBlastEnginePort25Canon
