...
declare @FinalUsers table(UserID int, UserName varchar(50), UserStatus varchar(20), Password varchar(50), DefaultClientGroupID int, DefaultClientID int,FirstName varchar(50), 
...
BEGIN
	...
	select u.UserID, UserName, u.Status, Password, DefaultClientGroupID, DefaultClientID, FirstName, LastName, Salt, EmailAddress, u.IsActive, AccessKey, IsAccessKeyValid,IsKMStaff
							,IsPlatformAdministrator, u.DateCreated, u.DateUpdated, u.CreatedByUserID, u.UpdatedByUserID ,'',sg.SecurityGroupName, ucsgm.IsActive,ISNULL(ucsgm.InactiveReason,''),sg.SecurityGroupID, case when ucsgm.inactivereason = 'Pending' then ISNULL(convert(varchar(100),sgoi.SetID),'') else '' end
