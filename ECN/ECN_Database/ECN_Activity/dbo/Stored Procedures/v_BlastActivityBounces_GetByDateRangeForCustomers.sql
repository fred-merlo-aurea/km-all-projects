CREATE PROCEDURE [dbo].[v_BlastActivityBounces_GetByDateRangeForCustomers]  
(
	@StartDate varchar(25),-- = '2/13/2013',
	@EndDate varchar(25),-- = '2/13/2013',
	@CustomerIDs varchar(255),-- = '1',
	@StringToFind varchar(255) = '0'
) AS 
BEGIN
	set @StartDate = @StartDate + ' 00:00:00'
	set @EndDate = @EndDate + ' 23:59:59'
	
	create table #SuppRecords (BlastID int, EmailID int, BounceTime datetime, GroupID int, EmailSubject varchar(255), EmailFrom varchar(255), CustomerID int)
	
	declare @SQLQuery varchar(max)
	set @SQLQuery = '
	INSERT INTO #SuppRecords
	SELECT 
		bab.BlastID, bab.EmailID, bab.BounceTime, b.GroupID, b.EmailSubject, b.EmailFrom, b.CustomerID
	FROM 
	...
	WHERE 
		...
		bab.BounceTime between ''' + @StartDate + ''' and ''' + @EndDate + ''' and
		bab.BounceMessage like ''%' + @StringToFind + '%'''
	exec (@SQLQuery)
	
	select '"' + e.EmailAddress + '"' as EmailAddress, '"' + sup.EmailSubject + '"' as EmailSubject, sup.BounceTime, '"' + g.GroupName + '"' as GroupName, '"' + sup.EmailFrom + '"' as EmailFrom, '"' + c.CustomerName + ' (' + CONVERT(varchar(20),sup.CustomerID) + ')"' as Customer, '"' + IsNull(edv.DataValue, '') + '"' as AccountID, '"' + IsNull(f.FolderName, '') + '"' as FolderName
	from
		#SuppRecords sup
...
--select * from BlastActivityBounces