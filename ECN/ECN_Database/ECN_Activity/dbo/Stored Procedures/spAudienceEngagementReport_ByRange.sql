CREATE  PROCEDURE [dbo].[spAudienceEngagementReport_ByRange](
	@groupID int,
	@clickPercentage int,
	@StartDate datetime,
	@EndDate datetime,
...
where b.groupID = @groupID and statuscode = 'sent' and  b.testblast='N' and  sendtime between @StartDate and @EndDate

-- print '0.1'+ ' / ' + convert(varchar(30), getdate() ,113 )


insert into @sd 
select bs.emailID, count(bs.sendID)  as sendcount , max(case when createdon between @StartDate and @EndDate then 1 else 0 end) as isNewlyCreated 
...
from  #emailactivitylog	e join 
	(
			...
			where 
					b.SendTime between @StartDate and @EndDate and 
					groupID = @groupID and 
					statuscode = 'sent' and  
					testblast='N'
...
Begin
	...
	union
	SELECT 2,  'Total Message Receivers', count(emailID), 'Has received messages at least once within the range ' + convert(varchar, @StartDate) + ' - ' + CONVERT(varchar, @EndDate) FROM #Emailactivitylog where sendcount > 0 and bouncecount = 0
	union
	SELECT 3,  'Best Clickers', count(emailID), 'Has clicked >=' + convert(varchar, @clickPercentage) + '% of the messages at least once within the range ' + convert(varchar, @StartDate) + ' - ' + CONVERT(varchar, @EndDate)  FROM #Emailactivitylog  where sendcount > 0 and bouncecount = 0 and clickcount > 0  and (clickcount*100)/sendcount >= @clickPercentage
	union
	SELECT 4,  'Active Clickers', count(emailID), 'Has clicked <' + convert(varchar, @clickPercentage) + '% of the messages at least once within the range ' + convert(varchar, @StartDate) + ' - ' + CONVERT(varchar, @EndDate) FROM #Emailactivitylog where sendcount > 0 and bouncecount = 0 and clickcount > 0 and (clickcount*100)/sendcount < @clickPercentage
	union
	select 5, 'Active Readers', count(emailID), 'Has opened but not clicked in the range ' + convert(varchar, @StartDate) + ' - ' + CONVERT(varchar, @EndDate) FROM #Emailactivitylog where sendcount > 0 and bouncecount = 0 and opencount > 0 and clickcount = 0
	union
	select 6, 'Became Inactive', count(emailID), 'Has opened or clicked, but not in the range ' + convert(varchar, @StartDate) + ' - ' + CONVERT(varchar, @EndDate) FROM #Emailactivitylog where sendcount > 0 and bouncecount = 0 and opencount = 0 and clickcount=0 and (HasOpenedBefore =1 or HasClickedBefore=1)
	union
	select 7, 'New Inactive', count(emailID), 'Has never opened or clicked and tenure  between the range ' + convert(varchar, @StartDate) + ' - ' + CONVERT(varchar, @EndDate) FROM #Emailactivitylog where sendcount > 0 and bouncecount = 0 and opencount = 0 and clickcount = 0 and isNewlyCreated =1
	union
	select 8, 'Old Inactive', count(emailID) , 'Has never opened or clicked and tenure between the range ' + convert(varchar, @StartDate) + ' - ' + CONVERT(varchar, @EndDate) FROM #Emailactivitylog where sendcount > 0 and bouncecount = 0 and  opencount = 0 and clickcount = 0 and HasOpenedBefore =0 and HasClickedBefore=0 and isNewlyCreated = 0
	) inn
