CREATE PROCEDURE [dbo].[spGetBlastGroupReportWithSuppressed]   
(  
	@ID int,   
	@CustomerID int,   
...
Begin      
		...
		Begin 
			...
			(
				SELECT 
					...
					bar.EMailID, e.EmailAddress, bar.ReferTime as ActionDate, bar.EmailAddress as ActionValue, 'EmailID='+CONVERT(VARCHAR,bar.EmailID) + '&GroupID=' + @groupID as URL 
				FROM 
		...
		Begin
			...
			begin  
				...
				begin
					...
					--SELECT e.EmailAddress as EmailAddress, sc.SupressedCode as Reason ,basupp.BlastsAlreadySent
					--FROM   BlastActivitySuppressed basupp with (NOLOCK)  join ecn5_communicator..Emails e with (NOLOCK) on basupp.emailid = e.emailID join @UDFFilter t on basupp.emailID = t.emailID  JOIN SuppressedCodes sc WITH (NOLOCK) on basupp.SuppressedCodeID = sc.SuppressedCodeID
					--where  basupp.blastID = @blastIDs    
			...
			Begin  
				...
				begin
					...
					insert into #tempSuppressed
						SELECT 
							ROW_NUMBER() OVER(ORDER BY SuppressID DESC) as 'RowNum', 
							e.EmailAddress as EmailAddress, sc.SupressedCode as Reason  ,BlastsAlreadySent 
						FROM 
							BlastActivitySuppressed basupp WITH (NOLOCK)
							join ecn5_communicator..Emails e WITH (NOLOCK) ON e.emailID = basupp.emailID 
							JOIN @UDFFilter t ON basupp.emailID = t.emailID
	...
	Begin		
		...
		Begin
			...
			Where BlastID in 
				(select blastid from ecn5_communicator..CampaignItemBlast where CampaignItemID = @ID and IsDeleted = 0) and
		...
		Begin 
			...
			(
				SELECT 
					ROW_NUMBER() OVER(ORDER BY ReferID DESC) as 'RowNum', 
					bar.EMailID, e.EmailAddress, ReferTime as ActionDate, bar.EmailAddress as ActionValue, 'EmailID='+CONVERT(VARCHAR,bar.EmailID) + '&GroupID=' + CONVERT(VARCHAR(100),ids.gid) as URL
				FROM 
					BlastActivityRefer bar WITH (NOLOCK)
					JOIN @b ids ON ids.bID = bar.blastID 
					join ecn5_communicator..Emails e WITH (NOLOCK) ON e.emailID = bar.emailID 
		...
		Begin
			...
			BEGIN
				...
				(
					SELECT 
						ROW_NUMBER() OVER(ORDER BY ActionDate DESC) as 'RowNum', 
						e.EMailID, e.EmailAddress,inn.ActionDate,inn.ActionValue,inn.ActionNotes, 'EmailID='+CONVERT(VARCHAR,e.EmailID) + '&GroupID=' + convert(varchar(10), gid) as  URL
					from ecn5_communicator..Emails e with (NOLOCK) JOIN 
					( 
		...
		Begin
			...
			Begin
				...
				begin  
					...
					(
						SELECT 
							ROW_NUMBER() OVER(ORDER BY UnsubscribeID DESC) as 'RowNum', 
							bau.EMailID, e.EmailAddress as EmailAddress, UnsubscribeTime as UnsubscribeTime, Comments as Reason, uc.UnsubscribeCode as SubscriptionChange, 'EmailID='+CONVERT(VARCHAR,bau.EmailID)+'&GroupID='+CONVERT(VARCHAR(100),ids.gid) AS URL
						FROM 
							BlastActivityUnSubscribes bau WITH (NOLOCK)
			...
			Begin
				exec spGetBlastGroupReport_for_Subscribe @ID, @FilterType, @ISP, @PageNo, @PageSize, @OnlyUnique 
			End
		End
		else if @ReportType = 'noclick'  
		Begin  
		...
		Begin 
			...
			begin  
				if(@FilterType = 'ALL')
				begin
					Select count(SuppressID) from BlastActivitySuppressed basupp with (NOLOCK) where basupp.BlastID in (select bID from @b)--  join @b ids ON ids.bID = basupp.blastID
					...
					insert into #tempSuppressed
						...
						FROM 
							...
							join ecn5_communicator..Emails e WITH (NOLOCK) ON e.emailID = basupp.emailID
						WHERE
							basupp.BlastID in (select bID from @b)
				...
				begin
					Select count(SuppressID) from BlastActivitySuppressed basupp with (NOLOCK) join SuppressedCodes sc on basupp.SuppressedCodeID = sc.SuppressedCodeID--join @b ids ON ids.bID = basupp.blastID
					WHERE	sc.SupressedCode = @FilterType and
							basupp.BlastID in (select bID from @b)
					...
					insert into #tempSuppressed
						...
						WHERE  
							sc.SupressedCode = @FilterType and
							basupp.BlastID in (select bID from @b)
			...
			Begin  
				...
				begin
					...
					WHERE  e.emailaddress like '%' + @ISP and basupp.BlastID in (select bID from @b)    
				...
				begin
					...
					from 
						...
						--join @b ids ON ids.bID = basupp.blastID
						join ECN_ACTIVITY..SuppressedCodes sc on basupp.SuppressedCodeID = sc.SuppressedCodeID
					WHERE  sc.SupressedCode = @FilterType AND e.emailaddress like '%' + @ISP and basupp.BlastID in (select bID from @b)      
					...
					insert into #tempSuppressed
						...
						FROM 
							...
							join ECN_ACTIVITY..SuppressedCodes sc on basupp.SuppressedCodeID = sc.SuppressedCodeID 
						WHERE  
							sc.SupressedCode = @FilterType and e.emailaddress like '%' + @ISP and basupp.BlastID in (select bID from @b)
