CREATE proc [dbo].[rpt_BlastActivitySocial_GetChartPreviews] 
(
...
Begin
	...
	BEGIN
		...
		SELECT sm.DisplayName, convert(int,ceiling(convert(float,SUM (CASE bas.SocialActivityCodeID WHEN 2 THEN 1 ELSE 0 END)) / convert(float,SUM (CASE bas.SocialActivityCodeID WHEN 1 THEN 1 ELSE 0 END)))) as Share 
        FROM
