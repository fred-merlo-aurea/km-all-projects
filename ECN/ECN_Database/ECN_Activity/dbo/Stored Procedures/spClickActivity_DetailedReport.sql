CREATE  PROCEDURE [dbo].[spClickActivity_DetailedReport](
	@BlastID int,    
...
BEGIN 	
	...
	Begin
		insert into #tempA    
		select	EmailID, ClickTime as ActionDate  from BlastActivityClicks bac with (nolock) 
	...
	Begin
		insert into #tempA    
		select	EmailID, ClickTime ActionDate  from BlastActivityClicks bac with (nolock)
		where	blastID = @BlastID and bac.URL = @LinkURL

	end

    if not exists(select top 1 GroupDatafieldsID from ecn5_communicator..groupdatafields with (nolock) where GroupDatafields.groupID = @GroupID)    
	Begin    
	...
	Begin    
		...
		SELECT  @StandAloneUDFs = STUFF(( SELECT DISTINCT '],[' + ShortName FROM ecn5_communicator..groupdatafields with (nolock) WHERE GroupID = @GroupID AND DatafieldSetID is null ORDER BY '],[' + ShortName FOR XML PATH('') ), 1, 2, '') + ']'
		DECLARE @TransactionalUDFs VARCHAR(2000)
		SELECT  @TransactionalUDFs = STUFF(( SELECT DISTINCT '],[' + ShortName FROM ecn5_communicator..groupdatafields with (nolock) WHERE GroupID = @GroupID AND DatafieldSetID > 0 ORDER BY '],[' + ShortName FOR XML PATH('') ), 1, 2, '') + ']'       
