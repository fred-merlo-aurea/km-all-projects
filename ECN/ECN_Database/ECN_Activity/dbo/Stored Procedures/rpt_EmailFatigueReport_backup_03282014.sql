CREATE Procedure [dbo].[rpt_EmailFatigueReport_backup_03282014]
...
(
	...
	FROM 
		...
		LEFT JOIN (SELECT * from ECN_Activity.dbo.BlastActivityOpens WITH (NOLOCK) WHERE OpenID >= @MinOpenId ) o ON s.blastid = o.blastid and s.emailid = o.emailid 
		LEFT JOIN (SELECT * FROM ECN_Activity.dbo.BlastActivityClicks  WITH (NOLOCK) WHERE ClickID >= @MinClickId  ) c ON s.blastid = c.blastid and s.emailid = c.emailid
	WHERE 
...
(
	...
	FROM 
		ECN_Activity.dbo.BlastActivitySends s WITH (NOLOCK)
		JOIN ECN5_COMMUNICATOR..Blast b WITH (NOLOCK) on b.BlastID = s.BlastID 
		JOIN (SELECT CampaignItemId,Blastid from ECN5_Communicator.dbo.campaignItemBlast  WITH (NOLOCK) WHERE BlastID IS NOT NULL 
				 UNION SELECT CampaignItemId,blastid from ECN5_Communicator.dbo.CampaignItemTestBlast WITH (NOLOCK)  WHERE BlastID IS NOT NULL) cib ON cib.BlastID = b.BlastID
