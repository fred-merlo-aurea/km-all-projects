...
namespace ECN.Tests.Helpers
{
    public static class ReflectionHelper
    {
        ...
        public static TField GetValue<TField>(object instance, string fieldName)
        {
            var type = instance.GetType();
            var fieldInfo = GetFieldInfoFromInstanceTypeByName(type, fieldName);
        ...
        public static TValue GetPropertyValue<TValue>(object instance, string fieldName)
        {
            var type = instance.GetType();
            var propertyInfo = GetPropertyInfoFromInstanceTypeByName(type, fieldName);
            return (TValue)propertyInfo.GetValue(instance);
        }

        public static TField GetValue<TField>(object instance, Type instanceType, string fieldName)
        {
            var fieldInfo = GetFieldInfoFromInstanceTypeByName(instanceType, fieldName);
            return (TField)fieldInfo.GetValue(instance);
        }
        ...
        public static FieldInfo GetFieldInfoFromInstanceByName(object instance, string fieldName)
        {
            return instance.GetType().GetFields(
                ...
                ReflectionFlags.Instance).FirstOrDefault(field => field.Name == fieldName);
        }
        ...
        public static PropertyInfo GetPropertyInfoFromInstanceTypeByName(Type instanceType, string fieldName)
        {
        ...
        public static object ExecuteMethod(object instance, string methodName, object[] parameters)
        {
            ...
            var dynMethod = instance.GetType().GetMethod(methodName,
                ReflectionFlags.NonPublic | ReflectionFlags.Instance);
        ...
        {
            var isMatched = true;
            var expectedProperties = expected.GetType().GetAllProperties();
            var actualProperties = actual.GetType().GetAllProperties();

            for (var i = 0; i < expectedProperties.Length; i++)
            {
                var expectedValue = expectedProperties[i].GetValue(expected) ?? string.Empty;
                var actualValue = actualProperties[i].GetValue(actual) ?? string.Empty;
        ...
        public static bool IsListContentMatched<T>(this List<T> expected, IList<T> actual)
        {
            var isMatched = true;
            for (var listIndex = 0; listIndex < expected.Count; listIndex++)
            {
                var expectedProperties = expected[listIndex].GetType().GetAllProperties();
                var actualProperties = actual[listIndex].GetType().GetAllProperties();

                for (var i = 0; i < expectedProperties.Length; i++)
                {
                    var expectedValue = expectedProperties[i].GetValue(expected[listIndex]) ?? string.Empty;
                    var actualValue = actualProperties[i].GetValue(actual[listIndex]) ?? string.Empty;
                    if (!expectedValue.Equals(actualValue))
                    {
                        Console.WriteLine(
                            $"Expected Property({expectedProperties[i].Name}) Value: {expectedValue} is not matched with actual value {actualValue} ");
