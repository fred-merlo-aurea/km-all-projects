...
{
    /// <summary>
    /// Summary description for ProcessLogin
    /// </summary>
    ...
    public class PharmaliveServices : System.Web.Services.WebService
    {
        ...
        public string Login(string ecnAccessKey, string username, string password)
        {
            ...
            if (ecnAccessKey == AccessKey)
            {
                ...
                cmd.CommandText = "SELECT emailID FROM ecn5_communicator..emails WHERE CustomerID = 2069 and emailaddress=@EmailAddress and password = @Password";
        ...
        public string getPremiumNewsletters(int userID)
        {
            ...
            try
            {
                string Pharmalive_SubscriptionGroupsIDs = DataFunctions.ExecuteScalar("select SubscriptionGroupIDs from SmartFormsHistory where SmartformID = " + ConfigurationManager.AppSettings["Pharmalive_SubscriptionGroups_SmartFormID"].ToString(), ConfigurationManager.AppSettings["com"].ToString()).ToString();
        ...
        [WebMethod(Description = "returns ALL subscribed newsletters groupIDs- delimited by comma(,)")]
