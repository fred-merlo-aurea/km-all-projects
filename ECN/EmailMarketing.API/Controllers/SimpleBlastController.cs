...
using APIModel = EmailMarketing.API.Models.SimpleBlast;
...
{   // aliasing inside name-space where the declarations will then be identical in each controller
    ...
    public class SimpleBlastController : SearchableApiControllerBase<APIModel, FrameworkModel>
    {
        private const string CampaignName = "Marketing Campaign";
        private const string BadScheduleErrorMessage = "bad schedule";
        private const string BlastResourceTypeName = "blast";

        /// <summary>
        /// Constructor, subscribes for AfterTransformation events.
        /// </summary>
        public SimpleBlastController()
            : base()
        {
        ...
        {
            FrameworkModel blast = BusinessLayerCommunicator.Blast.GetByBlastID(blastID, user, true) as FrameworkModel;
        ...
        private static readonly string[] SimpleBlastExposedProperties = new string[] {
            "BlastID",
        ...
        /// arrays by calling <see cref="BusinessLayerCommunicator.CampaignItemBlast.GetByBlastID"/></param>
        ...
        internal static APIModel InternalGetFiltersAndSmartSegments(int blastID, APIModel apiModel, KMPlatform.Entity.User user)
        {
            ECN_Framework_Entities.Communicator.CampaignItemBlast cib =
                    BusinessLayerCommunicator.CampaignItemBlast.GetByBlastID(blastID, user, true);
        ...
        public APIModel Get(int id)
        {
            ...
            if (null == model)
            {
                RaiseNotFoundException(id, BlastResourceTypeName);
            }
            return model;
        }
        #endregion GET
        ...
        public HttpResponseMessage Post([FromBody]APIModel model)
        {
            ...
            var setupInfo = SetupSchedule(model, DateTime.Now.AddSeconds(15));
            ...
            var campaignItemBlast = CreateCampaignItemBlast(model, campaignItem, out campaignItemBlastId);
        ...
        public HttpResponseMessage Put(int id, [FromBody]APIModel model)
        {
            ...
            if (null == campaignItemBlast || null == campaignItemBlast.BlastID)
            {
                RaiseNotFoundException(id, BlastResourceTypeName);
            }
            ...
            var setupInfo = SetupSchedule(model, null);
            var campaignItem = UpdateCampaignItem(model, campaignItemBlast, setupInfo);

            //update campaign item blast
            UpdateCompaignItemBlast(model, campaignItemBlast, blast.LayoutID, campaignItem.CampaignItemID);
        ...
        {
            ECN_Framework_Entities.Communicator.CampaignItemBlast ciBlast = BusinessLayerCommunicator.CampaignItemBlast.GetByBlastID(id, APIUser, false);
            ECN_Framework_Entities.Communicator.CampaignItemTestBlast ciTestBlast = BusinessLayerCommunicator.CampaignItemTestBlast.GetByBlastID(id, APIUser, false);
            if (ciBlast != null)
            {
                BusinessLayerCommunicator.CampaignItemBlastRefBlast.Delete(ciBlast.CampaignItemBlastID, APIUser);
                BusinessLayerCommunicator.CampaignItemBlast.Delete(ciBlast.CampaignItemID.Value, ciBlast.CampaignItemBlastID, APIUser);
            }
            else if (ciTestBlast != null)
            {
                BusinessLayerCommunicator.CampaignItemTestBlast.Delete(ciTestBlast.CampaignItemID.Value, ciTestBlast.CampaignItemTestBlastID, APIUser);
            }
            else
            {
                RaiseNotFoundException(id, BlastResourceTypeName);
        ...
        public IEnumerable<SearchResult> Search([FromBody] Models.SearchBase searchQuery)
        {
            return SearchBaseMethod(searchQuery, (controller, controllerContext, query) =>
            {
                ...
                return from x in BusinessLayerCommunicator.Blast.
                    GetBySearch(CustomerID, emailSubject, null, groupID, isTest, statusCode, modifiedFrom, modifiedTo, null, "", "", APIUser, false)
        ...
        public IEnumerable<Dictionary<string, string>> GetBlastBounceReport(int id, bool withDetail = false)
        {
            if (false == BusinessLayerCommunicator.Blast.Exists(id, CustomerID))
            {
                RaiseNotFoundException(id, BlastResourceTypeName);
            }
            ...
            return dt.ToSimpleDictionary(fieldsToUse);
        }
        ...
        public IEnumerable<Dictionary<string, string>> GetBlastClicksReport(int id, Models.Reports.ClickReportFilterType filterType = Models.Reports.ClickReportFilterType.All, bool withDetail = false)
        {
            if (false == BusinessLayerCommunicator.Blast.Exists(id, CustomerID))
            {
                ...
                throw new HttpResponseException(Request.CreateErrorResponse(
                    ...
                    Exceptions.APIResourceNotFoundException.GetAPIResourceNotFoundExceptionMessage(
                        Exceptions.APIResourceNotFoundException.Factory(BlastResourceTypeName, id))));
        ...
        public IEnumerable<Dictionary<string, string>> GetBlastDeliveryReport(DateTime fromDate, DateTime toDate)
        {

            DataTable dt = ECN_Framework_BusinessLayer.Activity.Report.BlastDelivery.Get(CustomerID.ToString(), fromDate, toDate);
            dt.TableName = "Report";
            return dt.ToSimpleDictionary();
        }
        ...
        public IEnumerable<Dictionary<string, string>> GetBlastOpensReport(int id, Models.Reports.ClickReportFilterType filterType = Models.Reports.ClickReportFilterType.All, bool withDetail = false)
        {
            if (false == BusinessLayerCommunicator.Blast.Exists(id, CustomerID))
            {
                RaiseNotFoundException(id, BlastResourceTypeName);
        ...
        /// 
        [Route("{id}/Report")]
        public Models.Reports.Blast.Report GetBlastReport(int id)
        {
            ...
            Models.Reports.Blast.Report blastReport = new Models.Reports.Blast.Report();
            ...
            {
                blastReport.Add(sendReportRow);
            }

            blastReport.Add(from row in dt.AsEnumerable()
                            where false == row["ActionTypeCode"].ToString().ToLower().Equals("send")
                            select MakeReportRow(blastReport, row));

            return blastReport;
        }
        Models.Reports.Blast.ReportRow MakeReportRow(Models.Reports.Blast.Report report, DataRow row)
        {
        ...
        [Route("{id}/report/ISP")]
        [HttpPost]
        public IEnumerable<Dictionary<string, string>> GetBlastReportByISP(int id, [FromBody] string[] ISPList)
        {
        ...
        public IEnumerable<Dictionary<string, string>> GetBlastUnsubscribeReport(int id)
        {

            if (false == BusinessLayerCommunicator.Blast.Exists(id, CustomerID))
            {
                RaiseNotFoundExceptionViaDirectThrow(id, BlastResourceTypeName);
            }
        ...
        int TestBlastGetCustomerCount(APIModel model)
        {
            ...
            return BusinessLayerCommunicator.Blast.GetBlastEmailsListCount(CustomerID, 0, model.GroupID, listFilters, "", "", true, APIUser);
        ...
        {
            var validationErrorList = new List<ECN_Framework_Common.Objects.ECNError>();
            ...
            var subjectOK = ValidateSubject(model, addError);
            var groupOK = ValidateGroup(model, addError);

            int? smartSegmentID = null;
            if (!BusinessLayerCommunicator.Layout.Exists(model.LayoutID, APICustomer.CustomerID))
            {
                addError("LayoutID unknown or inaccessible");
            }
            else
            {
                smartSegmentID = ValidateSmartSegmentId(model, addError);

                if (groupOK && subjectOK)
                {
                    ValidateLayout(model, addError);
                    ValidateTemplate(model, addError);
                }
            }
        ...
        private int? ValidateSmartSegmentId(APIModel model, Action<string> addError)
        {
            int? smartSegmentId = null;
            if (model.FilterID.HasValue && model.FilterID > 0)
            {
                if (!BusinessLayerCommunicator.Filter.Exists(model.FilterID.Value, APICustomer.CustomerID))
                {
                    if (BusinessLayerCommunicator.SmartSegment.SmartSegmentOldExists(model.FilterID.Value) && model.ReferenceBlasts.Any())
                    {
                        smartSegmentId = model.FilterID.Value;
                        if (!BusinessLayerCommunicator.Blast.RefBlastsExists(String.Join(",", model.ReferenceBlasts), APICustomer.CustomerID, model.SendTime))
                        {
        ...
        private static bool ValidateSubject(APIModel model, Action<string> addError)
        {
            var subjectOK = false;
        ...
        private bool ValidateGroup(APIModel model, Action<string> addError)
        {
            var groupOk = false;
            if (!BusinessLayerCommunicator.Group.Exists(model.GroupID, APICustomer.CustomerID))
            {
        ...
        private void ValidateTemplate(APIModel model, Action<string> addError)
        {
            try
            {
                var layout = BusinessLayerCommunicator.Layout.GetByLayoutID_NoAccessCheck(model.LayoutID, false);
                if (layout != null && layout.TemplateID.HasValue && layout.TemplateID.Value > 0)
                {
                    var template = BusinessLayerCommunicator.Template.GetByTemplateID_NoAccessCheck(layout.TemplateID.Value);
                    ...
                    BusinessLayerCommunicator.Group.ValidateDynamicStringsForTemplate(templateList, model.GroupID, APIUser);
                }
            }
        ...
        private void ValidateLayout(APIModel model, Action<string> addError)
        {
            try
            {
                var listLY = BusinessLayerCommunicator.Layout.ValidateLayoutContent(model.LayoutID);
                listLY.Add(model.EmailSubject.Trim().ToLower());
                BusinessLayerCommunicator.Group.ValidateDynamicStrings(listLY, model.GroupID, APIUser);
            }
            catch (ECN_Framework_Common.Objects.ECNException)
            {
        ...
        void ValidateModelState(APIModel model)
        {
            ...
            else
            {
                ...
                {
                    ECN_Framework_Entities.Communicator.Filter f = BusinessLayerCommunicator.Filter.GetByFilterID_NoAccessCheck(model.FilterID.Value);
        ...
        void ValidateSimpleBlastSchedule(Models.SimpleBlastSchedule schedule)
        {
            ...
            var errors = new List<ECN_Framework_Common.Objects.ECNError>();
        ...
        private Entities.BlastSetupInfo SetupSchedule(APIModel model, DateTime? defaultSendTime)
        {
            ...
            if (model.Schedule != null)
            {
                ...
                if (null == setupInfo)
                {
                    throw new ECNException(
                        new List<ECNError>()
                        {
                            new ECNError(Enums.Entity.Blast, Enums.Method.Validate, BadScheduleErrorMessage)
                        });
                }
                ...
                else if (defaultSendTime.HasValue)
                {
                    model.SetSendTime(defaultSendTime.Value);
                }
        ...
        private string ConvertEmailSubject(string emailSubject)
        {
            ...
            if (emailSubject.Any(x => x > 255))
            {
                ...
                var mc = uniRegex.Matches(emailSubject);
                foreach (Match m in mc)
                {
                    emailSubject = emailSubject.Replace(m.Value, EmojiFunctions.GetUnicode(m.Value));
                }

                var mc2 = OtherRegex.Matches(emailSubject);
        ...
        private ECN_Framework_Entities.Communicator.BlastAbstract CreateBlastAndApplyToNewModel(APIModel model, ECN_Framework_Entities.Communicator.CampaignItem campaignItem, ECN_Framework_Entities.Communicator.CampaignItemBlast campaignItemBlast)
        {
            if (model.IsTestBlast)
            {
                //create campaign item test blast and actual blast
                return CreateTestBast(model, campaignItem);
            }
            else
            {
        ...
        private ECN_Framework_Entities.Communicator.BlastAbstract CreateTestBast(APIModel model, ECN_Framework_Entities.Communicator.CampaignItem campaignItem)
        {
            var testBlast = new ECN_Framework_Entities.Communicator.CampaignItemTestBlast
            {
            ...
            BusinessLayerCommunicator.CampaignItemTestBlast.Insert(testBlast, APIUser);
            ...
            var blast = BusinessLayerCommunicator.Blast.GetByCampaignItemTestBlastID(testBlast.CampaignItemTestBlastID, APIUser, false);
            if (blast != null && model.FilterID > 0)
            {
        ...
        private static void CreateCampaignItemBlastFilter(APIModel model, int cibID)
        {
            if (model.FilterID > 0 || model.SmartSegmentID.HasValue)
            {
                var blastFiler = new ECN_Framework_Entities.Communicator.CampaignItemBlastFilter
                {
                    CampaignItemBlastID = cibID,
                    IsDeleted = false
                };

                if (model.SmartSegmentID.HasValue && model.ReferenceBlasts.Any())
                {
                    blastFiler.SmartSegmentID = model.SmartSegmentID.Value;
                    blastFiler.RefBlastIDs = String.Join(",", model.ReferenceBlasts);
                }
                else
                {
                    blastFiler.FilterID = model.FilterID;
                }
                BusinessLayerCommunicator.CampaignItemBlastFilter.Save(blastFiler);
            }
        }

        private Entities.CampaignItem CreateCampaignItem(
            APIModel model,
            Entities.Campaign campaign,
            Entities.BlastSetupInfo setupInfo,
            out string campaignItemName)
        {
            campaignItemName = $"{model.EmailSubject} {DateTime.Now.ToString("MM/dd/yyyy hh:mm:ss.fff tt")}";

            var campaignItem = new Entities.CampaignItem
            {
                CustomerID = CustomerID,
                CreatedUserID = APIUser.UserID,
        ...
        private Entities.Campaign CreateCampaign()
        {
            var campaign = BusinessLayerCommunicator.Campaign.GetByCampaignName(CampaignName, APIUser, false);
            ...
            if (campaign == null || campaign.CampaignID <= 0)
            {
                ...
                BusinessLayerCommunicator.Campaign.Save(campaign, APIUser);
            }

            return campaign;
        }

        private Entities.CampaignItemBlast CreateCampaignItemBlast(APIModel model, Entities.CampaignItem campaignItem, out int campaignItemBlastId)
        {
            var campaignItemBlast = new Entities.CampaignItemBlast
            {
                CampaignItemID = campaignItem.CampaignItemID,
                CustomerID = CustomerID,
                GroupID = model.GroupID,
            ...
            campaignItemBlastId = BusinessLayerCommunicator.CampaignItemBlast.Save(campaignItemBlast, APIUser);
            return campaignItemBlast;
        }

        private void UpdateCompaignItemBlast(
            APIModel model,
            Entities.CampaignItemBlast campaignItemBlast,
            int? blastLayoutId,
            int campaignItemId)
        {
            campaignItemBlast.CampaignItemID = campaignItemId;
            campaignItemBlast.GroupID = model.GroupID;
            campaignItemBlast.UpdatedUserID = APIUser.UserID;
            campaignItemBlast.EmailSubject = model.EmailSubject;
            campaignItemBlast.LayoutID = blastLayoutId;

            var itemBlastId = BusinessLayerCommunicator.CampaignItemBlast.Save(campaignItemBlast, APIUser);

            if (model.FilterID > 0)
            {
                BusinessLayerCommunicator.CampaignItemBlastFilter.DeleteByCampaignItemBlastID(itemBlastId);
                var blastFilter = new Entities.CampaignItemBlastFilter
                {
                    CampaignItemBlastID = itemBlastId,
                    FilterID = model.FilterID,
                    IsDeleted = false
                };
                BusinessLayerCommunicator.CampaignItemBlastFilter.Save(blastFilter);
            }
            else
            {
                BusinessLayerCommunicator.CampaignItemBlastFilter.DeleteByCampaignItemBlastID(itemBlastId);
        ...
        private Entities.CampaignItem UpdateCampaignItem(
            APIModel model,
            Entities.CampaignItemBlast campaignItemBlast,
            Entities.BlastSetupInfo setupInfo)
        {
            var campaignItemId = BusinessLayerCommunicator.CampaignItem.GetByCampaignItemID(campaignItemBlast.CampaignItemID.Value, APIUser, false);

            campaignItemId.UpdatedUserID = APIUser.UserID;
            campaignItemId.FromEmail = model.EmailFrom;
            campaignItemId.FromName = model.EmailFromName;
            campaignItemId.ReplyTo = model.ReplyTo;

            if (null != setupInfo && setupInfo.SendTime.HasValue)
            {
                campaignItemId.SendTime = setupInfo.SendTime;
                campaignItemId.BlastScheduleID = setupInfo.BlastScheduleID.Value;
            }

            BusinessLayerCommunicator.CampaignItem.Save(campaignItemId, APIUser);
