    ...
    public class FriendlyExceptionsAttribute : OrderedExceptionFilterAttribute
    {
        ...
        public override void OnException(HttpActionExecutedContext actionExecutedContext)
        {
            ...
            if (CatchUnfilteredExceptions || null != exceptionDefinition)
            {
                ...
                // Bug 44542:Change validation error message for DeleteByEmailAddress
                ECNException ex = actionExecutedContext.Exception as ECNException;
                if (ex != null && ex.Message.Contains("ECN_Framework_Common.Objects.ECNException") && ex.ErrorList != null && ex.ErrorList.Count > 0)
                {
                    friendlyMessage = string.Empty;
                    foreach (var msg in ex.ErrorList)
                    {
                        friendlyMessage += msg.ErrorMessage;
                    }
                }
