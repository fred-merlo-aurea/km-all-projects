...
namespace EmailMarketing.API.Exceptions
{
    ...
    public class APIResourceNotFoundException : Exception
    {
        ...
        {
            if (apiResourceNotFoundException.StackTrace.Contains("API.Controllers.EmailGroupController.DeleteSubscriber"))
                return "Resource not found.  Email " + apiResourceNotFoundException.Data[Strings.Errors.APIResourceNotFoundExceptionResourceKey] + " not found in group " + apiResourceNotFoundException.Data[Strings.Errors.APIResourceNotFoundExceptionIDKey] + ".";
            else
