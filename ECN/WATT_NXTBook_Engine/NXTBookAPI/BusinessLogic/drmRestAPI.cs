...
namespace WATT_NXTBook_Engine.NXTBookAPI.BusinessLogic
{
    public class drmRestAPI
    {
        ...
        public static drmProfile GetProfile(string apiKey, string subID, string email)
        {
            HttpWebRequest request = null;
            drmError errorObject = null;

            string postParams = "https://drm.nxtbook.com/fx/drm/rest/GetAccessProfile?apikey=" + apiKey + "&subscriptionid=" + subID + "&email=" + email;
            ...
            using (HttpWebResponse response = (HttpWebResponse)request.GetResponse())
            {
                ...
                if (!error)
                {
                    ...
                    using (Stream s = response.GetResponseStream())
                    {
                        StreamReader sr = new StreamReader(s);
                        responseData = sr.ReadToEnd();
                        profile = JsonConvert.DeserializeObject<drmProfile>(responseData);
