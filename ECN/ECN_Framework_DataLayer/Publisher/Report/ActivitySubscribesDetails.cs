...
namespace ECN_Framework_DataLayer.Publisher.Report
{
    ...
    public class ActivitySubscribesDetails
    {
        ...
        {
            List<ECN_Framework_Entities.Publisher.Report.ActivitySubscribesDetails> retList = new List<ECN_Framework_Entities.Publisher.Report.ActivitySubscribesDetails>();

            string sqlQuery = "sp_GetActivity_SubscribesDetails";
            ...
            using (SqlDataReader rdr = DataFunctions.ExecuteReader(cmd, DataFunctions.ConnectionString.Publisher.ToString()))
            {
                if (rdr != null)
                {
                    var builder = DynamicBuilder<ECN_Framework_Entities.Publisher.Report.ActivitySubscribesDetails>.CreateBuilder(rdr);
                    while (rdr.Read())
                    {
                        ECN_Framework_Entities.Publisher.Report.ActivitySubscribesDetails x = builder.Build(rdr);
                        retList.Add(x);
                    }
                    rdr.Close();
                    rdr.Dispose();
