...
namespace ECN_Framework_DataLayer.Publisher.Report
{
    ...
    public class ActivityVisitPerPage
    {
        ...
        {
            List<ECN_Framework_Entities.Publisher.Report.ActivityVisitPerPage> retList = new List<ECN_Framework_Entities.Publisher.Report.ActivityVisitPerPage>();

            string sqlQuery = "sp_GetActivity_VisitsPerPage";
            ...
            using (SqlDataReader rdr = DataFunctions.ExecuteReader(cmd, DataFunctions.ConnectionString.Publisher.ToString()))
            {
                if (rdr != null)
                {
                    var builder = DynamicBuilder<ECN_Framework_Entities.Publisher.Report.ActivityVisitPerPage>.CreateBuilder(rdr);
                    while (rdr.Read())
                    {
                        ECN_Framework_Entities.Publisher.Report.ActivityVisitPerPage x = builder.Build(rdr);
                        retList.Add(x);
                    }
                    rdr.Close();
                    rdr.Dispose();
