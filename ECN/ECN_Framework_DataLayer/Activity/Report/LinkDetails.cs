...
namespace ECN_Framework_DataLayer.Activity.Report
{
    ...
    public class LinkDetails
    {
        ...
        {
            List<ECN_Framework_Entities.Activity.Report.LinkDetails> retList = new List<ECN_Framework_Entities.Activity.Report.LinkDetails>();
            string sqlQuery = "splinkDetailsReport";
            ...
            using (SqlDataReader rdr = ECN_Framework_DataLayer.DataFunctions.ExecuteReader(cmd, ECN_Framework_DataLayer.DataFunctions.ConnectionString.Activity.ToString()))
            {
                if (rdr != null)
                {
                    var builder = DynamicBuilder<ECN_Framework_Entities.Activity.Report.LinkDetails>.CreateBuilder(rdr);
                    while (rdr.Read())
                    {
                        ECN_Framework_Entities.Activity.Report.LinkDetails x = builder.Build(rdr);
                        retList.Add(x);
                    }
                    rdr.Close();
                    rdr.Dispose();
