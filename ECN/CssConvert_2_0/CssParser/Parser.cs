...
namespace CssConvert.CssParser
{
    public class Parser
    {
        ...
        void attrib(out CssConvert.CssParser.Attribute atb)
        {
            atb = new CssConvert.CssParser.Attribute();
            string quote = null;
            string ident = null;
        ...
        private delegate void GetOutStringValue(out string stringValue);

        private void AssignValueAndType(Term termObject, GetOutStringValue getOutString, out string paramString, TermType termType)
        {
            if (termObject == null)
            {
            ...
            {
                throw new ArgumentNullException(nameof(getOutString));
            }

            getOutString(out paramString);
            termObject.Value = paramString;
            termObject.Type = termType;
        }

        private void term(out Term trm)
        {
            trm = new Term();
            var val = string.Empty;
            string ident;

            if (la.kind == Seven || la.kind == Eight)
            {
                AssignValueAndType(trm, QuotedString, out val, TermType.String);
            }
            else if (la.kind == Nine)
            {
                AssignValueAndType(trm, URI, out val, TermType.Url);
            }
            else if (la.kind == FortySix)
            {
                Get();
                identity(out ident);
                trm.Value = $"U\\{ident}";
                trm.Type = TermType.Unicode;
            }
            else if (la.kind == ThirtyThree)
            {
                AssignValueAndType(trm, HexValue, out val, TermType.Hex);
            }
            else if (StartOf(Three))
            {
                AssignValueAndType(trm, identity, out ident, TermType.String);

                TermStartOfFifteen(trm);
                ...
                if (la.kind == Ten)
                {
                    ...
                    Expression exp;
                    expr(out exp);
                    var func = new Function { Name = trm.Value, Expression = exp };
                    trm.Value = null;
                    trm.Function = func;
            ...
            else if (StartOf(Seventeen))
            {
                TermStartOfSeventeen(trm, val);
            }
            else
            {
                SynErr(FiftySeven);
        ...
        private void TermStartOfSeventeen(Term trm, string val)
        {
            var valStringBuilder = new StringBuilder(val);

            if (trm == null)
            {
                throw new ArgumentNullException(nameof(trm));
            }

            if (la.kind == TwentyTwo || la.kind == TwentyNine)
            {
                if (la.kind == TwentyTwo)
                {
                    Get();
                    trm.Sign = Minus;
                }
            ...
            {
                Get();
                valStringBuilder.Append(t.val);
            }

            if (la.kind == ThirtyFour)
            {
                Get();
                valStringBuilder.Append(t.val);
            ...
            TermStartOfEighteen(trm, valStringBuilder);

            trm.Value = valStringBuilder.ToString();
            trm.Type = TermType.Number;
        }

        private void TermStartOfEighteen(Term trm, StringBuilder valStringBuilder)
        {
            string ident;
            ...
            if (valStringBuilder == null)
            {
                throw new ArgumentNullException(nameof(valStringBuilder));
            }

            if (StartOf(Eighteen))
            {
                if (la.val.Equals(NValue, StringComparison.OrdinalIgnoreCase))
                {
                    Expect(TwentyThree);
                    valStringBuilder.Append(t.val);
        ...
        private void TermStartOfFifteen(Term trm)
        {
            if (StartOf(Fifteen))
            {
                var trmValueStringBuilder = new StringBuilder(trm.Value);

                while (la.kind == ThirtyFour || la.kind == ThirtySix || la.kind == FortyThree)
                {
                    string ident;

                    if (la.kind == FortyThree)
                    {
                        Get();
                        trmValueStringBuilder.Append(t.val);
                        ...
                        identity(out ident);
                        trmValueStringBuilder.Append(ident);
                    }
                    else if (la.kind == ThirtyFour)
                    {
                        Get();
                        trmValueStringBuilder.Append(t.val);
                        identity(out ident);
                ...
                trm.Value = trmValueStringBuilder.ToString();
            }
        }
