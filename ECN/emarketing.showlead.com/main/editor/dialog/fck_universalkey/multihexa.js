...
tracer("fond",posUniKeyLeft,posUniKeyTop,'<img src="images/keyboard_layout.gif" width=404 height=152 border="0"><br />',"sign")
/*touches*/
...
/*zones reactives*/
tracer("masque",posUniKeyLeft,posUniKeyTop,'<img src="images/00.gif" width=404 height=152 border="0" usemap="#unikey">')
...
function arkey(a) {
		...
		if (event.shiftKey) {
			...
			if (key==192) {cont.value += 'ّ';entry=false;}
			if (key==219) {cont.value += '<';entry=false;}
			if (key==221) {cont.value += '>';entry=false;}
		} else {
			if (key==186) {cont.value += 'ك';entry=false;}
			if (key==188) {cont.value += 'و';entry=false;}
			if (key==190) {cont.value += 'ز';entry=false;}
			if (key==191) {cont.value += 'ظ';entry=false;}
			if (key==192) {cont.value += 'ذ';entry=false;}
			if (key==219) {cont.value += 'ج';entry=false;}
			if (key==221) {cont.value += 'د';entry=false;}
			if (key==222) {cont.value += 'ط';entry=false;}
		}
		
		
		return entry;
	}
var obj = MM_findObj('uni_area'); if (obj) obj.onkeydown = arkey