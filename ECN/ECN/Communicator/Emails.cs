...
namespace ecn.communicator.classes {
    ...
    public class Emails : DatabaseAccessor {
		...
		public static Emails GetEmailByID(int emailID) {
            ...
            email.Password = Convert.ToString(row["Password"]);
        ...
        public string EmailAddress() {
            ...
            if(ID() != 0) {
                try {
                    _email_address = DataFunctions.ExecuteScalar("communicator", "Select EmailAddress from Emails where EmailID=" + ID()).ToString();
                } catch {
        ...
        public int CustomerID() {
            ...
            if(ID() != 0) {
                try {
                    _customer_id = Convert.ToInt32(DataFunctions.ExecuteScalar("communicator", "Select CustomerID from Emails where EmailID=" + ID()).ToString());
                } catch {
                }
            }
            return _customer_id;
        ...
        /// Returns the MobileNumber from the Database
        ...
        public string MobileNumber()
        {
            if (ID() != 0)
            {
                try
                {
                    _mobile = DataFunctions.ExecuteScalar("communicator", "Select Mobile from Emails where EmailID=" + ID()).ToString();
                }
                catch
                {
                }
        ...
        /// Checks to see if an phone number and email belongs to a customer. Updates self if so.

        /// <param name="email_address">Email Address</param>
        /// <param name="customer_id">Customer</param>
        /// <param name="email_address">Mobile Number</param>
        public void GetMobile(string email_address, int customer_id, string mobile_number)
        {
            object eid = DataFunctions.ExecuteScalar("communicator",
            "Select EmailID from Emails where EmailAddress = '" + email_address + "' and CustomerID = " + customer_id + " and Mobile = '" + mobile_number+"'");
        ...
        /// Ensures that an mobile number exists in the database.

        /// <returns>number of created numbers (1 or 0)</returns>
        public int EnsureMobile()
        {
            GetMobile(_email_address, _customer_id, _mobile);
            if (ID() == 0)
            {
                SqlCommand insert_cmd = new SqlCommand(null, GetDbConnection());
                insert_cmd.CommandText = "Insert INTO Emails (EmailAddress,CustomerID, Mobile) VALUES (@email_address,@customer_id, @mobile_number);SELECT @@IDENTITY";

                insert_cmd.Parameters.Add("@email_address", SqlDbType.VarChar, 255, "EmailAddress").Value = EmailAddress();
                insert_cmd.Parameters.Add("@customer_id", SqlDbType.Int, 4, "CustomerID").Value = CustomerID();
                insert_cmd.Parameters.Add("@mobile_number", SqlDbType.VarChar, 12, "Mobile").Value = MobileNumber();
                insert_cmd.Prepare();
                int new_id = Convert.ToInt32(insert_cmd.ExecuteScalar());
                ID(new_id);
                return 1;
