...
{
    public partial class DownloadPanel_EV : DownloadPanelV
    {
            ...
            set
            {
                ...
                if (Session["SubscriptionID"] != null)
                {
                    ...
                    if (Subscribers.Any())
                    {
                        ...
                        {
                            int count = PubSubscriptions.GetUniqueProductEmailsCount(clientconnections, SubscribersQueries, PubIDs.First());
                            txtDownloadUniqueCount.Text = count.ToString();
                            hfTotalUniqueCount.Value = count.ToString(); 
                        }
                        else
                        {
                            int count = Subscriber.GetUniqueEmailsCount(clientconnections, SubscribersQueries);
                            txtDownloadUniqueCount.Text = count.ToString();
                            hfTotalUniqueCount.Value = count.ToString();
                        }
                    }
        ...
        public void LoadControls()
        {
            ...
            if (ViewType == Enums.ViewType.ProductView)
            {
                int UniqueCount = PubSubscriptions.GetUniqueProductEmailsCount(clientconnections, SubscribersQueries, PubIDs.First());
                txtDownloadUniqueCount.Text = UniqueCount.ToString();
                hfTotalUniqueCount.Value = UniqueCount.ToString();
            }
            else
            {
                int UniqueCount = Subscriber.GetUniqueEmailsCount(clientconnections, SubscribersQueries);
                txtDownloadUniqueCount.Text = UniqueCount.ToString();
                hfTotalUniqueCount.Value = UniqueCount.ToString();
            }
        }
        ...
        protected override void ResetPopupControls()
        {
            hfTotalUniqueCount.Value = string.Empty;
        }
            ...
            if (rbDownloadAll.Checked || rbDownload.Checked)
            {
                if (KM.Platform.User.HasAccess(UserSession.CurrentUser, KMPlatform.Enums.Services.UAD, KMPlatform.Enums.ServiceFeatures.UADExport, KMPlatform.Enums.Access.Download))
                {
                    if (rbDownloadAll.Checked)
                    {
                        if (ViewType == Enums.ViewType.ProductView)
                        {
                            dtSubscription = Subscriber.GetProductDimensionSubscriberData_EV(clientconnections, SubscribersQueries, stdColumnList, PubIDs, ResponseGroupIDList, ResponseGroupDescIDList, PubSubscriptionsExtMapperValueList, customColumnList, BrandID, false);
                        }
                        else
                        {
                            dtSubscription = Subscriber.GetSubscriberData_EV(clientconnections, SubscribersQueries, stdColumnList, MasterGroupColumnList, MasterGroupColumnDescList, SubscriptionsExtMapperValueList, customColumnList, BrandID, PubIDs, false, ViewType == Enums.ViewType.RecencyView ? true : false);
                        }
                    }
                    else
                    {
                        ...
                        if (ViewType == Enums.ViewType.ProductView)
                        {
                            dtSubscription = Subscriber.GetProductDimensionSubscriberData_EV(clientconnections, SubscribersQueries, stdColumnList, PubIDs, ResponseGroupIDList, ResponseGroupDescIDList, PubSubscriptionsExtMapperValueList, customColumnList, BrandID, true);
                        }
                        else
                        {
                            dtSubscription = Subscriber.GetSubscriberData_EV(clientconnections, SubscribersQueries, stdColumnList, MasterGroupColumnList, MasterGroupColumnDescList, SubscriptionsExtMapperValueList, customColumnList, BrandID, PubIDs, true, ViewType == Enums.ViewType.RecencyView ? true : false);
                        }
                    }
