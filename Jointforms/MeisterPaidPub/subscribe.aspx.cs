    ...
    public partial class subscribe : System.Web.UI.Page
    {
        ...
        public void updateGridView()
        {
            ...
            foreach (GridViewRow row in grdMagazines.Rows)
            {
                if (row.RowType == DataControlRowType.DataRow)
                {
                    ...
                    Label lblAirMailIntPrice1 = (Label)row.FindControl("lblAirMailIntPrice1");
                    Label lblPubCode = (Label)row.FindControl("lblPubCode");
                    Label lblMessage = (Label)row.FindControl("lblMessage");
                    Label lblAllowAirmail = (Label)row.FindControl("lblAllowAirmail");
                    DropDownList ddlQuantity = (DropDownList)row.FindControl("drpQuantity");
                    CheckBox chkAirMail = (CheckBox)row.FindControl("chkAirMail");
                    switch (Convert.ToInt32(ddlQuantity.SelectedItem.Value))
                    {
                        case 1:
                            if (countryCode.ToUpper() == "UNITED STATES OF AMERICA" || countryCode.ToUpper() == "UNITED STATES")
                            {
                                ...
                                chkAirMail.Visible = false;
                                lblAirMailIntPrice1.Visible = false;
                            ...
                            {
                                if(lblAllowAirmail.Text.Equals("True"))
                                    chkAirMail.Visible = true;
                                else
                                    chkAirMail.Visible = false;

                                if (!chkAirMail.Checked)
                                {
                                    lblUsPrice1.Visible = false;
                                    lblUsPrice2.Visible = false;
                                    lblCanPrice1.Visible = false;
                                ...
                                else
                                {
                                    ...
                                    lblIntPrice1.Visible = false;
                                    lblIntPrice2.Visible = false;
                                    lblAirMailIntPrice1.Visible = true;
                                    lblAirMailIntPrice1.Text = String.Format("{0:0.00}", (Convert.ToDecimal(lblAirMailIntPrice1.Text) / 1M).ToString());
                                }
                            }
                            break;
                        case 2:
        ...
        public void CalculatePrice()
        {
            ...
            foreach (GridViewRow row in grdMagazines.Rows)
            {
                ...
                {                   
                    ShowIntLabel(row, totalLabel, chkPrint, quantity, chkAirMail);
                }
            }
        }

        private void ShowIntLabel(GridViewRow row, Label totalLabel, CheckBox chkPrint, int quantity, CheckBox chkAirMail)
        {
            Label intLabel = new Label();
            if (quantity == 1 && chkAirMail.Checked==false)
            {
                intLabel = (Label)row.FindControl("lblIntPrice1");
            }
            else if (quantity == 1 && chkAirMail.Checked == true)
            {
                intLabel = (Label)row.FindControl("lblAirMailIntPrice1");
            }
            else if (quantity == 2)
            {
                ...
                foreach (GridViewRow row in grdMagazines.Rows)
                {
                    ...
                    if (chkPrint.Checked == true)
                    {
                        ...
                        if (countryCode == "UNITED STATES OF AMERICA" || countryCode == "UNITED STATES" || countryCode == "CANADA")
                        {
                            ...
                            sb.Append("&user_t_AirmailPaid=False");
                        }
                        else
                        {
                            sb.Append("&user_t_State=" + txtState.Text);

                            if (chkAirMail.Checked && chkAirMail.Visible)
                                sb.Append("&user_t_AirmailPaid=True");
                            else
                                sb.Append("&user_t_AirmailPaid=False");
                        }
        ...
        private bool ValidateCreditCard()
        {
            try
            {
                ...
                ppPay.AddLineItem("Meister Media", "Paid Publications", itemDesc, 1, Convert.ToDecimal(ppPay.Amount), false);
        ...
        protected void btnReset_Click(object sender, EventArgs e)
        {
            ...
            foreach (GridViewRow grdRow in grdMagazines.Rows)
            {
                CheckBox chkAirMail = (CheckBox)grdRow.FindControl("chkAirMail");
                chkAirMail.Checked = false;
            }
        ...
        protected void grdMagazines_RowDataBound(object sender, GridViewRowEventArgs e)
        {
            ...
            Label lblAirMailIntPrice1 = (Label)e.Row.FindControl("lblAirMailIntPrice1");

            CheckBox chkAirMail = (CheckBox)e.Row.FindControl("chkAirMail");
            ...
            if (lblPubCode != null)
            {
                if (countryCode.ToUpper() == "MEXICO" && lblPubCode.Text.Equals("PDH"))
                {
                    e.Row.Visible = false;
                }
                else
                ...
                if (quantity == 1)
                {
                    ...
                    {
                        if (chkAirMail.Visible && chkAirMail.Checked)
                            totalPrice = Convert.ToDouble(dtpubs.Rows[rowIndex]["intpriceone"]);
                        else
                            totalPrice = Convert.ToDouble(dtpubs.Rows[rowIndex]["airmailintpriceone"]);
                    }
                }
                else
                {
        ...
        protected void chkAirMail_CheckedChanged(object sender, EventArgs e)
        {
            updateGridView();
            CalculatePrice();
        }
