    ...
    public class ccPayPal : ccProcessing
    {
        ...
        public override bool ValidateCard(Publication pub)
        {
            try
            {
                ...
                if (ppPay.ErrorList.Count > 0)
                    ValidatedMessage = ppPay.ErrorList[0].LongMessage;
            ...
            {
                WriteErrorLog(ex.ToString() + ex.StackTrace);
                return false;
            }
        }
        ...
        {
            System.IO.File.WriteAllText(WebConfigurationManager.AppSettings["LogFilePath"].ToString() + "transactionLog.txt", "DateTime:" + DateTime.Now.ToString() + " | RequestValues:" + ppPay.RequestString + " | ResponseString:" + ppPay.ResponseString);
        }

        private void WriteErrorLog(string msg)
        {
            System.IO.File.WriteAllText(WebConfigurationManager.AppSettings["LogFilePath"].ToString() + "errorlog.txt", "DateTime:" + DateTime.Now.ToString() + " |Message:" + msg);
        }
    }
}
